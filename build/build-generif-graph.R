...
if(0) {
    R <- read.csv("../files/generifs_basic.txt",sep="\t",check.names=FALSE)
    dim(R)
    head(R)

    gene <- unique(unlist(as.list(org.Hs.egSYMBOL)))
    rif <- as.character(R[2,5])
    scan.genes <- function(rif) {
        rif <- gsub("\\(|\\)|\\[|\\]","",rif)
        rif<- strsplit(gsub("[,._]"," ",rif),split=" ")[[1]]
        rif2 <- setdiff(toupper(rif),c("WAS","AN","IV"))
        intersect(gene, rif2)
    }

    rifs <- as.character(R[,5])
    rif.genes <- lapply(rifs[1:1000], scan.genes)
