...
echo "nohup docker build -f docker/Dockerfile --no-cache -t $image:testing . > docker.out &"
echo "nohup docker build -f docker/Dockerfile --no-cache --build-arg TAG=develop -t $image:develop . > docker.out &"
