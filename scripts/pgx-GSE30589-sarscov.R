...
rda.file="../data-extra/GSE30589-sarscov.pgx"
...
ngs$description = "GSE30589. Severe acute respiratory syndrome coronavirus envelope protein regulates cell stress response and apoptosis (DeDiego, PLoS Pathog 2011)."
...
if(PROCESS.DATA) {
    ...
    geo <- getGEO("GSE30589", GSEMatrix=TRUE, getGPL=TRUE)
    attr(geo, "names")
    X <- exprs(geo[[1]])
    head(X)[,1:4]
    max(X)
