...
if(PROCESS.DATA) {
    ...
    counts = fread("~/Projects/Data/GSE/GSE115469_Data.csv",nrow=-1000)
    dim(counts)
    head(counts)[,1:10]
    ...
    if(0) {
        ## show histogram of library size
        lib.size <- colSums(counts)
        hist(lib.size, breaks=200)

        ## show histogram of library size
        lib.size <- log10(colSums(counts))
        hist(lib.size, breaks=200)

        min.size <- mean(lib.size) - 2*sd(lib.size)
        max.size <- mean(lib.size) + 2*sd(lib.size)
        abline( v=c(min.size, max.size), col="red", lty=2)
        table( (lib.size > min.size & lib.size < max.size))
