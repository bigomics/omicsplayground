...
on a geneset level, also called gene set enrichment (GSE) analysis.

```{r}

gs_infotext =
    "Similar to the differential gene expression analysis, users can perform differential expression analysis on a geneset level in this page, which is also referred as gene set enrichment (GSE) analysis. The platform has more than 50.000 genesets (or pathways) in total that are divided into 30 geneset collections such as Hallmark, MSigDB, KEGG and GO. Users have to specify which comparison they want to visually analyze employing a certain geneset collection. It is also possible to filter out some genesets by logFC and FDR thresholds. The platform performs enrichment analyses using up to seven different methods including Spearman rank correlation, GSVA, ssGSEA, Fisher's exact test, GSEA, camera and fry, and reports the combined result.

<br><br>The combined result from the methods is displayed under <strong>Enrichment Table tab</strong> (bottom half), where for each geneset the meta.q corresponds to the highest q value provided by the methods and the number of stars indicate how many methods identified the geneset as significant (q < 0.05). The table is interactive; users can sort it by logFC, meta.q and starts. Additionally, the list of genes in that geneset are displayed in the second table on the right.

<br><br>Under <strong>Top Enriched tab</strong>, for a selected comparison, the top 10 differentially enriched geneses (pathways) are displayed. Under the <strong>Plots tab</strong>, for each geneset in the enrichment table, when it is selected, a volcano plot of genes contained in that genesest and a barplot of expressions per sample group are displayed. Under the <strong>Compare tab</strong>, for a selected geneset from the enrichment table, users can compare the differential expression status of that geneset for all other comparisons. Finally, volcano plots of genesets for all comparisons are displayed under the <strong>Volcano (all) tab</strong>. This allows users to have an overall picture across comparisons at the same time.
...
br();br();br();br();br();
actionLink("gs_options", "Options", icon=icon("cog", lib = "glyphicon"))
br();br();
conditionalPanel(
    "input.gs_options % 2 == 1",
    tagList(
        checkboxInput('gs_ungroup','ungroup (Plots)',FALSE),
        checkboxInput('gs_top10','top 10 (Table)',FALSE)
    )
)
...
if(PRO.VERSION) {
    conditionalPanel(
        "input.gs_options % 2 == 1",
        tagList(
            textInput("gs_filter","Filter:"), br(), 
            checkboxGroupInput('gs_method','Test method:', choices=NULL)
        )
    )
}
...
if(DEV.VERSION) {
    conditionalPanel(
        "input.gs_options % 2 == 1",
        tagList(
            radioButtons('gs_lfcmethod','Score method:', choices=c("fc","gs"))
        )
    )
}
...
enrich_top_enrichedFUNC <- reactive({
...
enrich_top_enriched_module <- plotModule(
    id="enrich_top_enriched", func=enrich_top_enrichedFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=95
)

##output$enrich_top_enriched     <- enrich_top_enriched_module$render
##output$enrich_top_enriched_pdf <- enrich_top_enriched_module$pdf
output <- attachModule(output, enrich_top_enriched_module)

fillCol(flex = c(NA,1),
                 enrich_top_enriched_module$button,
                 plotOutput("enrich_top_enriched", height = "100%", width = "100%"))
...
enrich_subplot1_module <- plotModule(
    id="enrich_subplot1", func=enrich_subplot1FUNC,
    info.text = "This is an example", pdf.width=10, pdf.height=10, res=70
)
##output$enrich_subplot1     <- enrich_subplot1_module$render
##output$enrich_subplot1_pdf <- enrich_subplot1_module$pdf
output <- attachModule(output, enrich_subplot1_module)

##----------------------------------------------------------------------
## 1: Gene set activation {data-width=200}
##----------------------------------------------------------------------
enrich_subplot2FUNC <- reactive({
...
enrich_subplot2_module <- plotModule(
    id="enrich_subplot2", func=enrich_subplot2FUNC,
    info.text = "This is an example", pdf.width=10, pdf.height=10, res=70
)

##output$enrich_subplot2     <- enrich_subplot2_module$render
##output$enrich_subplot2_pdf <- enrich_subplot2_module$pdf
output <- attachModule(output, enrich_subplot2_module)

##----------------------------------------------------------------------
## 2: Gene expression {data-width=200}
##----------------------------------------------------------------------
enrich_subplot3FUNC <- reactive({
...
enrich_subplot3_module <- plotModule(
    id="enrich_subplot3", func=enrich_subplot3FUNC,
    info.text = "This is an example", pdf.width=10, pdf.height=10, res=70
)
##output$enrich_subplot3     <- enrich_subplot3_module$render
##output$enrich_subplot3_pdf <- enrich_subplot3_module$pdf
output <- attachModule(output, enrich_subplot3_module)

##----------------------------------------------------------------------
## 3: Gene - gene set correlation
##----------------------------------------------------------------------
enrich_subplot4FUNC <- reactive({
...
enrich_subplot4_module <- plotModule(
    id="enrich_subplot4", func=enrich_subplot4FUNC,
    info.text = "This is an example", pdf.width=10, pdf.height=10, res=70
)

##output$enrich_subplot4     <- enrich_subplot4_module$render
##output$enrich_subplot4_pdf <- enrich_subplot4_module$pdf
output <- attachModule(output, enrich_subplot4_module)

fillRow(height = '100%', flex = c(1,1,1,1), 
        fillCol(flex = c(NA,1),
                enrich_subplot1_module$button,
                plotOutput("enrich_subplot1", height = "100%", width = "100%")),
        fillCol(flex = c(NA,1),
                enrich_subplot2_module$button,
                plotOutput("enrich_subplot2", height = "100%", width = "100%")),
        fillCol(flex = c(NA,1),
                enrich_subplot3_module$button,
                plotOutput("enrich_subplot3", height = "100%", width = "100%")),
        fillCol(flex = c(NA,1),
                enrich_subplot4_module$button,
                plotOutput("enrich_subplot4", height = "100%", width = "100%"))
)
```   
...
enrich_cont_compareFUNC <- reactive({
...
enrich_cont_compare_module <- plotModule(
    id="enrich_cont_compare", func=enrich_cont_compareFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=95
)

##output$enrich_cont_compare     <- enrich_cont_compare_module$render
##output$enrich_cont_compare_pdf <- enrich_cont_compare_module$pdf
output <- attachModule(output, enrich_cont_compare_module)

fillCol(flex = c(NA,1),
                 enrich_cont_compare_module$button,
                 plotOutput("enrich_cont_compare", height = "100%", width = "100%"))
...
enrich_volcano_allFUNC <- reactive({
    ##renderPlotly({
    require(metap)
    ngs = inputData()
    if(is.null(ngs)) return(NULL)
...
enrich_volcano_all_module <- plotModule(
    id="enrich_volcano_all", func=enrich_volcano_allFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=95
)

## output$enrich_volcano_all     <- enrich_volcano_all_module$render
## output$enrich_volcano_all_pdf <- enrich_volcano_all_module$pdf
output <- attachModule(output, enrich_volcano_all_module)

fillCol(flex = c(NA,1),
        enrich_volcano_all_module$button,
        plotOutput("enrich_volcano_all", height = "100%", width = "100%"))
...
enrich_volcano_methodsFUNC <- reactive({
    ##renderPlotly({
    ngs <- inputData()    
    if(is.null(ngs)) return(NULL)
    if(is.null(input$gs_features)) return(NULL)
...
enrich_volcano_methods_module <- plotModule(
    id="enrich_volcano_methods", func=enrich_volcano_methodsFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=95
)
##output$enrich_volcano_methods     <- enrich_volcano_methods_module$render
##output$enrich_volcano_methods_pdf <- enrich_volcano_methods_module$pdf
output <- attachModule(output, enrich_volcano_methods_module)

fillCol(flex = c(NA,1),
                 enrich_volcano_methods_module$button,
                 plotOutput("enrich_volcano_methods", height = "100%", width = "100%"))
...
enrich_genemap_module <- plotModule(
    id="enrich_genemap", func=enrich_genemapFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=65
)

##output$enrich_genemap     <- enrich_genemap_module$render
##output$enrich_genemap_pdf <- enrich_genemap_module$pdf
...
enrich_concept_module <- plotModule(
    id="enrich_concept", func=enrich_conceptFUNC,
    info.text = "This is an example", pdf.width=22, pdf.height=6, res=95
)

##output$enrich_concept     <- enrich_concept_module$render
##output$enrich_concept_pdf <- enrich_concept_module$pdf
output <- attachModule(output, enrich_concept_module)

fillCol(flex = c(NA,1),
                 enrich_concept_module$button,
                 plotOutput("enrich_concept", height = "100%", width = "100%"))
...
enrich_gseatableFUNC <- reactive({
    ...
    selectmode = "single"
    ##if(PRO.VERSION) selectmode <- "multiple"
    selectmode
...
enrich_gseatable_text = "This is an example"
enrich_gseatable_module <- tableModule(
    id="enrich_gseatable", func=enrich_gseatableFUNC,
    info.text = enrich_gseatable_text
)

output$enrich_gseatable <- enrich_gseatable_module$render
output$enrich_gseatable_csv <- enrich_gseatable_module$csv


enrich_genetable_text = "This is an example"
enrich_genetable_module <- tableModule(
    id="enrich_genetable", func=enrich_genetableFUNC,
    info.text = enrich_genetable_text,
    server=FALSE
)
output$enrich_genetable <- enrich_genetable_module$render
output$enrich_genetable_csv <- enrich_genetable_module$csv


fillRow(flex = c(2,0.10,1),         
...
enrich_fctable_module <- tableModule(
    id="enrich_fctable", func=enrich_fctableFUNC,
    info.text = "This is an example"
)
##output$enrich_fctable <- enrich_fctable_module$render
##output$enrich_fctable_csv <- enrich_fctable_module$csv
output <- attachModule(output, enrich_fctable_module)


fillCol(flex = c(NA,1),
        enrich_fctable_module$button,
        dataTableOutput("enrich_fctable", height = "100%", width = "100%")) 
...
enrich_FDRtable_text = "This is an example"

enrich_FDRtable_module <- tableModule(
    id="enrich_FDRtable", func=enrich_FDRtable.FUNC,
    info.text = enrich_FDRtable_text,
   ,title='Number of significant gene sets'
)
##output$enrich_FDRtable <- enrich_FDRtable_module$render
