...
<br> **Enrichment Analysis**. Perform differential expression analysis
on a geneset level, also called geneset enrichment analysis.


```{r}
htmlOutput("curdata5", class="current-data")
...
<br><br>For a tutorial on Enrichment Analysis, see the video below:<br><br>
<center><iframe width='500' height='333' src='https://www.youtube.com/embed/t90x9TlW6Y0' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></center>
")

tipify( actionLink("gs_info", "Info", icon = icon("info-circle")),
...
## require(rintrojs)
## suppressMessages(introjsUI())
## actionButton("gs_intro","start intro")
## gs_steps <-  data.frame(
##     element = c(NA,"#gs_intro","#gs_contrast"),
##     intro = c("Hello world","This is a button","Select here your contrast")
## )
## ##observeEvent(input$gs_intro, { introjs(session, options = list(steps=gs_steps)) })
## introjs(session, options = list(steps=gs_steps))
...
```{r}
tipify( selectInput("gs_contrast","Contrast:", choices=NULL),
       "Select a contrast of interest for the analysis.", placement="top")
tipify( selectInput("gs_features","Gene set collection:", choices=NULL, multiple=FALSE),
        "Choose a specific gene set collection for the analysis.", placement="top")
FDR.VALUES2 = c(1e-9,1e-6,1e-3,0.01,0.05,0.1,0.2,0.5,1)
fillRow( flex=c(1,1),
        tipify( selectInput("gs_fdr","FDR", choices=FDR.VALUES2, selected=0.2),
               "Set the false discovery rate (FDR) threshold.", placement="top"),
        tipify( selectInput("gs_lfc","logFC threshold", choices=c(0,0.2,0.5,1,2,5), selected=0.2),
...
conditionalPanel(
    ...
    tipify(actionLink("gs_options", "Options", icon=icon("cog", lib = "glyphicon")),
           "Toggle advanced options.", placement="top"),
    br(),
    conditionalPanel(
        "input.gs_options % 2 == 1",
        tagList(
            tipify( checkboxGroupInput('gs_method','Statistical methods:', choices=NULL),
...
if(DEV.VERSION) {
    ...
    tagList(
        ...
        tipify( radioButtons('gs_lfcmethod','Score method: (dev)', choices=c("fc","gs")),
...
observe({
    ...
    sel2 = head(intersect(GSET.DEFAULTMETHODS,gset.methods),3)
...
enrich_topEnriched_module <- plotModule(
    id="enrich_topEnriched", func=enrich_topEnriched.RENDER,
...
fillCol(
    moduleWidget(enrich_topEnriched_module)
)


```   

> **Top enriched gene sets.** Enrichment plots of the top differentially enriched gene sets (up and down). Black vertical bars indicate the rank of genes in the gene set in the sorted list metric. The green curve corresponds to the 'running statistics' of the enrichment score.
...
enrich_subplot1_module <- plotModule(
    id="enrich_subplot1", func=enrich_subplot1.RENDER,
...
enrich_subplot2_module <- plotModule(
    id="enrich_subplot2", func=enrich_subplot2.RENDER,
    info.text = enrich_subplot2_text,
    pdf.width=6, pdf.height=6, res=80,
    options = tagList( tipify( checkboxInput('gs_ungroup1','ungroup samples',FALSE),
...
enrich_subplot3_module <- plotModule(
    id="enrich_subplot3", func=enrich_subplot3.RENDER,
    info.text = enrich_subplot3_text,
    pdf.width=6, pdf.height=6, res=80,
    options = tagList( tipify( checkboxInput('gs_ungroup2','ungroup samples',FALSE),
           "Ungroup samples in the plot", placement="top", options = list(container = "body"))),
    title = "Expression barplot", label="c"
)
enrich_subplot4_module <- plotModule(
    id="enrich_subplot4", func=enrich_subplot4.RENDER,
...
enrich_compare_module <- plotModule(
    id="enrich_compare", func=enrich_compare.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_compare_module$button,
    ## plotOutput("enrich_compare", height = "100%", width = "100%")
    moduleWidget(enrich_compare_module)
)


```   

> **Enrichment across contrasts.** Enrichment plots for the selected gene set (in **Table I**) across multiple contrasts. The figure allows to quickly compare the enrichment of a certain gene set across all other comparisons.
...
enrich_volcanoAll_module <- plotModule(
    id="enrich_volcanoAll", func=enrich_volcanoAll.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_volcanoAll_module$button,
    ## plotOutput("enrich_volcanoAll", height = "100%", width = "100%")
    moduleWidget(enrich_volcanoAll_module)
)

``` 

> **Volcano plots for all contrasts.** Simultaneous visualisation of volcano plots of gene set enrichment across all contrasts. Volcano-plot are plotting enrichment score versus significance on the x and y axes, respectively. Experimental contrasts showing better statistical significance will show volcano plots with 'higher' wings.
...
enrich_volcanoMethods_module <- plotModule(
    id="enrich_volcanoMethods", func=enrich_volcanoMethods.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_volcanoMethods_module$button,
    ## plotOutput("enrich_volcanoMethods", height = "100%", width = "100%")
    moduleWidget(enrich_volcanoMethods_module)
)

``` 

> **Volcano plots for all methods.** Simultaneous visualisation of volcano plots of gene sets for different enrichment methods. Methods showing better statistical significance will show volcano plots with 'higher' wings.
...
enrich_genemap_module <- plotModule(
    id="enrich_genemap", func=enrich_genemap.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_genemap_module$button,
    ## plotOutput("enrich_genemap", height = "100%", width = "100%")
    moduleWidget(enrich_genemap_module)
)

```

> **Co-activation heatmap.** Clustered heatmap of top most correlated gene sets and their shared genes. The top gene sets most correlated with the selected gene set (in Table I) are shown. Red corresponds to overexpression, blue to downregulation of the gene. 
...
enrich_gseatable.RENDER <- reactive({
    ...
    DT::datatable(rpt,
                  ...
                  options=list(
                      ...
                      scrollX = TRUE, scrollY = 250, scroller=TRUE, deferRender=TRUE
                  )) %>%
...
enrich_gseatable_opts = tagList(
    tipify( checkboxInput('gs_top10','top 10 gene sets',FALSE),
    ...
    tipify(checkboxInput('gs_showqvalues','show q-values',FALSE),
           "Show q-values of all statistical methods in the table.", 
           placement="top", options = list(container = "body"))    
)

enrich_gseatable_module <- tableModule(
    id="enrich_gseatable", func=enrich_gseatable.RENDER,
...
enrich_genetable_module <- tableModule(
    id="enrich_genetable", func=enrich_genetable.RENDER,
...
fillRow(
    ...
    ##         enrich_gseatable_module$button,
    ##         br(),
    ##         dataTableOutput("enrich_gseatable", height = "100%", width = "100%")),
    ## fillCol(flex = c(NA,0.01, 1),
    ##         enrich_genetable_module$button,
    ##         br(),
    ##         dataTableOutput("enrich_genetable", height = "100%", width = "100%"))
    moduleWidget(enrich_gseatable_module, outputFunc="dataTableOutput"),
    br(),
    moduleWidget(enrich_genetable_module, outputFunc="dataTableOutput")        
)

```

> **Enrichment tables**. **(I)** Table summarizing the statistical results of the gene set enrichment analysis for selected contrast. The number of stars indicate how many methods identified the geneset significant. **(II)** Table showing the fold-change, statistics and correlation of the genes in the selected gene set.
...
enrich_fctable_module <- tableModule(
    id="enrich_fctable", func=enrich_fctable.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_fctable_module$button,
    ## dataTableOutput("enrich_fctable", height = "100%")
    moduleWidget(enrich_fctable_module, outputFunc="dataTableOutput")
) 

```

> **Enrichment for all contrasts.** Table summarizing the enrichment for all gene sets across all contrasts. The column `fc.var` corresponds to the variance of the gene set across all contrasts.
...
enrich_FDRtable_module <- tableModule(
    id="enrich_FDRtable", func=enrich_FDRtable.RENDER,
...
fillCol(
    ## flex = c(NA,1),
    ## enrich_FDRtable_module$button,
    ## dataTableOutput("enrich_FDRtable", height = "100%", width = "100%")
    moduleWidget(enrich_FDRtable_module, outputFunc="dataTableOutput")
) 
```

> **FDR table.** Number of significant gene sets versus different FDR thresholds, for all contrasts and all methods. The blue color denote the number of downregulated genes, the red color for upregulated genes.
