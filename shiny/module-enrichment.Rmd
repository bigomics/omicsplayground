...
enrich_gseatable.RENDER <- reactive({
    ...
    if("GS" %in% colnames(rpt)) rpt$GS = shortstring(rpt$GS,120)
    ...
    if(length(jj)>0) rpt[,jj] = apply(rpt[,jj,drop=FALSE],2,shortstring,120)
...
enrich_genetable.RENDER <- reactive({
    ...
    if(!is.null(rpt) && nrow(rpt)>0 ) {
        ...
        jj = which( sapply(rpt,is.character) |  sapply(rpt,is.factor) )
        if(length(jj)>0) rpt[,jj] = apply(rpt[,jj,drop=FALSE],2,shortstring,40)
    } else {
...
enrich_fctable.RENDER <- reactive({
    ...
    gs <- shortstring(rownames(F),1,120)
