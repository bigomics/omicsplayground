...
fillRow( flex=c(1,1),
        ...
        selectInput("gs_lfc","logFC threshold", choices=c(0,0.2,0.5,1,2,5), selected=0.5)
        )
br();br();br();br();
...
getGeneSetTable <- reactive({
    ...
    outputs = NULL
    gsmethod0 = colnames(unclass(mx$fc))
    ...
    if(!is.null(gsfilt) && !(gsfilt %in% c(""," "))) {
        ...
        if(substring(gsfilt,1,1)=="#" && nchar(gsfilt.gene)>=2 && length(jj)>0 ) {
            ## search for gene sets that includes this
    ...
    if(FALSE && length(gsmethod)==1 && any(grepl("^gsea",gsmethod)) ) {
        ...
        ## y1 <- abs(pmax(ngs$model.parameters$exp.matrix[,comp],0)) ## y=+1
        ## y2 <- abs(pmin(ngs$model.parameters$exp.matrix[,comp],0)) ## y=-1
        ## AveExpr1 <- (ngs$gsetX[rownames(mx),names(y1)] %*% y1/sum(y1!=0))[,1]
        ## AveExpr0 <- (ngs$gsetX[rownames(mx),names(y2)] %*% y2/sum(y2!=0))[,1]
        s1 <- names(which(ngs$model.parameters$exp.matrix[,comp]>0))
        s0 <- names(which(ngs$model.parameters$exp.matrix[,comp]<0))
        AveExpr1 <- rowMeans(ngs$gsetX[rownames(mx),s1])
        AveExpr0 <- rowMeans(ngs$gsetX[rownames(mx),s0])
        
        ##meta.fc <- meta[,"fc"]
        meta.fc <- AveExpr1 - AveExpr0
        
        ## adjust means to reflect foldchange
        mean0 <- (AveExpr0 + AveExpr1)/2
        AveExpr1 <- mean0 + meta.fc/2
        AveExpr0 <- mean0 - meta.fc/2

        rpt = data.frame( size=gset.size, logFC=meta.fc, meta.q=meta[,"qv"],
                         stars, AveExpr0=AveExpr0, AveExpr1=AveExpr1)
        if(PRO.VERSION) {
            ## add extra p/q value columns
            colnames(qv) <- paste0("q.",colnames(qv))
            rpt <- cbind( rpt, qv)
        }
        rownames(rpt) = rownames(mx)
    }  else {
...
geneDetails <- reactive({
    ...
    limma1 = sapply(ngs$gx.meta$meta[[comp]][,c("fc","p","q")],function(x) x[,"trend.limma"])
    ...
    genes = intersect(genes, ngs$genes[rownames(ngs$X),"gene_name"])
