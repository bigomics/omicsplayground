...
```{r}
tipify( actionLink("gx_info", "Info", icon = icon("info-circle")),
...
<br><br>For a recommended tutorial on Differential Expression analysis, see the video below:<br><br><center><iframe width='500' height='333' src='https://www.youtube.com/embed/dvozWzolVI8' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></center>"
...
conditionalPanel(
    ...
    conditionalPanel(
        ...
        tagList(
            tipify( checkboxGroupInput('gx_testmethod','Statistical methods:',
...
expr_plots_volcano_module <- plotModule(
    id="expr_plots_volcano", func=expr_plots_volcano.RENDER,
...
expr_plots_maplot_module <- plotModule(
    id="expr_plots_maplot", func=expr_plots_maplot.RENDER,
...
expr_plots_topgenesbarplot_module <- plotModule(
    id="expr_plots_topgenesbarplot", func=expr_plots_topgenesbarplot.RENDER,
...
expr_plots_topfoldchange.RENDER <- reactive({
    ...
    m1 <- ifelse(nch > 12, 10, 6)
    m1 <- ifelse(nch > 30, 15, m1)
...
expr_topgenes_opts = tagList(
    ...
    tipify( checkboxInput('gx_ungroup','ungroup samples',FALSE),
...
expr_topgenes_module <- plotModule(
    id="expr_topgenes", func=expr_topgenes.RENDER, options=expr_topgenes_opts,
...
fillCol(
    moduleWidget(expr_topgenes_module)
)

```   

> **Top differentially expressed genes.** Expression barplots of the top most differentially (both positively and negatively) expressed genes for the selected contrast.
...
expr_volcanoAll_module <- plotModule(
    id="expr_volcanoAll", func=expr_volcanoAll.RENDER,
...
fillCol(
    moduleWidget(expr_volcanoAll_module)
)

```

> **Volcano plot for all contrasts.** Simultaneous visualisation of volcano plots of genes for all contrasts. Experimental contrasts with better statistical significance will show volcano plots with 'higher' wings.
...
expr_volcanoMethods_module <- plotModule(
    id="expr_volcanoMethods", func=expr_volcanoMethods.RENDER,
...
fillCol(
    moduleWidget(expr_volcanoMethods_module)
)

```

> **Volcano plot for all statistical methods.** Simultaneous visualisation of volcano plots of genes by multiple differential expression methods for the selected contrast. Methods showing better statistical significance will show volcano plots with 'higher' wings.
...
expr_genetable.RENDER <- reactive({
    ...
    DT::datatable( res, rownames=FALSE,
                  ...
                  options=list(
                      ...
                      scrollX = TRUE, scrollY = 260, scroller=TRUE, deferRender=TRUE
                  )  ## end of options.list 
                  ) %>%
...
expr_gsettable_module <- tableModule(
    id="expr_gsettable", func=expr_gsettable.RENDER,
...
fillRow(
    ...
    moduleWidget(expr_genetable_module, outputFunc="dataTableOutput"),
    br(),
    moduleWidget(expr_gsettable_module, outputFunc="dataTableOutput")        
)

```

> **Differential expression tables**. **(I)** Statistical results of the the differential expression analysis for selected contrast. The number of stars indicate how many statistical methods identified the gene significant. **(II)** Correlation and enrichment value of gene sets that contain the gene selected in Table I.
...
expr_fctable_module <- tableModule(
    id="expr_fctable", func=expr_fctable.RENDER,
...
fillCol(
    moduleWidget(expr_fctable_module, outputFunc="dataTableOutput")
)

```

> **Differential expression (fold-change) across all contrasts.** The column `fc.var` corresponds to the variance of the fold-change across all contrasts.
...
expr_FDR_table.RENDER <- reactive({
...
expr_FDR_table_text = "The <strong>FDR table</strong> tab reports the number of significant genes at different FDR thresholds for all contrasts within the dataset."

expr_FDR_table_module <- tableModule(
    id="expr_FDR_table", func=expr_FDR_table.RENDER,
    info.text = expr_FDR_table_text,
    title='Number of significant genes'
)
output <- attachModule(output, expr_FDR_table_module)
fillCol(
    moduleWidget(expr_FDR_table_module, outputFunc="dataTableOutput")
)

```

> **Number of significant genes versus FDR.** This table reports the number of significant genes at different FDR thresholds for all contrasts and methods. This enables to quickly see which methods are more sensitive. The left part of the table (in blue) correspond to the number of significant down-regulated genes, the right part (in red) correspond to the number of significant overexpressed genes.
