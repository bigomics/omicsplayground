...
dataviewTSNEPlotModuleUI <- function(id, label='', height=c(600,800)) {
...
dataviewTSNEPlotModuleServer <- function(id, data, filterStates, watermark=FALSE)
{
    moduleServer( id, function(input, output, session) {
        ...
        plot_data <- shiny::reactive({
            ...
            if(!is.null(filterStates$search_gene) && filterStates$search_gene!="") {
                gene <- filterStates$search_gene
            }
            ...
            grp <- NULL
            filt.groupby <- filterStates$data_groupby

            message("[dataviewTSNEPlotModuleServer] dim.data = ",paste(dim(data)))
            message("[dataviewTSNEPlotModuleServer] filterStates$data_groupby = ",filt.groupby)
