...
<br><br>In addition, it generates a classification tree using top features and provides expression boxplots by phenotype classes for features present in the tree. The platform can also provide a survival tree analysis using top features and provides expression boxplots by phenotype classes for features present in the tree."
    ...
    output$inputsUI <- renderUI({
        ui <- tagList(
            ...
            tipify(actionButton(ns("pdx_runbutton"), label="Run"),
    ...
    pdx_importance_module <- plotModule(
        "pdx_importance", ns=ns, func=pdx_importance.RENDER,
    ...
    pdx_heatmap_module <- plotModule(
        "pdx_heatmap", ns=ns, func=pdx_heatmap.RENDER,
    ...
    pdx_decisiontree_module <- plotModule(
        "pdx_decisiontree", ns=ns, func=pdx_decisiontree.RENDER,
    ...
    pdx_boxplots_module <- plotModule(
        "pdx_boxplots", ns=ns, func=pdx_boxplots.RENDER,
    ...
    output$pdx_biomarker_UI <- renderUI({
        fillCol(
            ...
            fillRow(
                ...
                fillCol(
                    flex = c(0.7,1),
                    moduleWidget(pdx_importance_module, ns=ns),
                    moduleWidget(pdx_heatmap_module, ns=ns)
                ),
                br(), ## spacer
                fillCol(
                    flex = c(1,0.9), 
                    moduleWidget(pdx_decisiontree_module, ns=ns),
                    moduleWidget(pdx_boxplots_module, ns=ns)
                )
            ),
            br(),
    ...
    mix_plotLoadings_module <- plotModule(
        "mix_plotLoadings", ns=ns, func=mix_plotLoadings.RENDER,
    ...
    mix_hivePlot_module <- plotModule(
        "mix_hivePlot", ns=ns, func=mix_hivePlot.RENDER,
    ...
    mix_plotSNF_module <- plotModule(
        "mix_plotSNF", ns=ns, func=mix_plotSNF.RENDER,
    ...
    mix_cimDiablo_module <- plotModule(
        "mix_cimDiablo", ns=ns, func=mix_cimDiablo.RENDER,
    ...
    mix_buttons <- plotModuleButtons(
        "mix_level", ns=ns, 
        text="Multi-level variable selection. This module is based on the DIABLO framework of the MixOmics R/package.",
    ...
    output$pdx_multilevel_UI <- renderUI({    
        fillCol(
            ...
            fillRow(
                ...
                fillCol(
                    flex = c(0.9,0.06,1),
                    moduleWidget(mix_plotLoadings_module, ns=ns),
                    br(),
                    fillRow(
                        flex = c(1.2,1),
                        ##plotOutput('mix_plotDiablo'),
                        moduleWidget(mix_hivePlot_module, ns=ns),
                        moduleWidget(mix_plotSNF_module, ns=ns)
                        ##plotOutput('mix_circosPlot')
                    )
                ),
                br(),
                moduleWidget(mix_cimDiablo_module, ns=ns)
            ),
