...
output$kegg_graph <- renderImage({
    ...
    cat("kegg_graph: copy",outfile,"to",KEGGIMG,"\n")
...
kegg_buttons1 = plotModuleButtons(
    "kegg_graph", text=kegg_info1, just.info=TRUE, 
...
fillRow(
    ...
    fillCol(
        ...
        br(),
        moduleWidget(kegg_table_module, outputFunc="dataTableOutput")
    ),
    br(),  ## horizontal space
    moduleWidget(kegg_actmap_module)
)
...
fillRow(
    ...
    fillCol(
        flex = c(2,1),
        moduleWidget(GO_network_module, outputFunc="visNetworkOutput"),
        moduleWidget(GO_table_module, outputFunc="dataTableOutput")
    ),
    moduleWidget(GO_actmap_module)
)
...
fillRow(
    ...
    fillCol(
        ...
        fillRow(
            flex=c(2.2,1),
            moduleWidget(dsea_enplots_module),
            moduleWidget(dsea_moaplot_module)
        ),
        br(),  ## vertical space
        moduleWidget(dsea_table_module, outputFunc="dataTableOutput")        
    ),
    moduleWidget(dsea_actmap_module)        
)
...
enrich_wordtsne_module <- plotModule(
    ...
    options = tagList(
        tipify(radioButtons("enrich_wordtsne_algo","Clustering algorithm:", choices=c("tsne","umap"),inline=TRUE),
...
enrich_wordcloud_opts = tagList(
    tipify(selectInput("enrich_wordcloud_exclude","Exclude words:", choices=NULL, multiple=TRUE),
           "Paste a keyword to exclude it from the plot.", placement="top", options = list(container = "body")),
    tipify(selectInput("enrich_wordcloud_colors","Colors:", choices=c("Blues","Greys","Accent","Dark2"), multiple=FALSE),
