...
<br> **Functional Analysis**. Perform specialized functional analysis
to understand biological functions including GO, KEGG, drug
connectivity mapping, and ligand-receptor interaction analysis.


```{r}
tipify( actionLink("fa_info", "Info", icon = icon("info-circle") ),
       "Show more information about this module.")

fa_infotext =
    "This module performs specialized pathway and drug enrichment analysis.

<br><br><a href='kegg pathway'>KEGG</a> is a collection of manually curated pathways representing the current knowledge of molecular interactions, reactions and relation networks as pathway maps. Within the KEGG pathway section, each pathway is scored for the selected contrast profile and reported in the table.  The scoring is performed by considering the total number of genes in the pathway (n), the number of genes in the pathway supported by the contrast profile (k), the ratio of k/n, and the ratio of |upregulated or downregulated genes|/k. Additionally, the table contains the list of the upregulated and downregulated genes for each pathway and a q value from the Fisher’s test for the overlap. The table is interactive; enabling user to sort on different variables. It is possible to visualize the pathway map by clicking on the pathway in the table, where genes are colored according to their upregulation (red) or downregulation (blue) in the contrast profile. Another important feature of the platform is that it provides an activation-heatmap including the comparison of activation levels of pathways (or pathway keywords) across multiple contrast profiles. This facilitates to quickly see and detect the similarities between profiles in certain pathways.


<br><br><a href='http://www.geneontology.org'>Gene Ontology</a> (GO) defines functional concepts/classes and their relationships as a hierarchical graph. The GO database provides a computational representation of the current knowledge about roles of genes for many organisms in terms of molecular functions, cellular components and biological processes. All the features described under the KEGG pathway tab, such as scoring the gene sets and drawing an activation-heatmap, can be performed for the GO database under the GO graph tab. Instead of pathway maps, an annotated graph structure provided by the GO database is potted for every selected gene set.

<br><br> The <a href='https://portals.broadinstitute.org/cmap/'>Drug Connectivity Map</a> correlates your signature with more than 5000 known drug profiles from the L1000 database, and shows the top N=10 similar and opposite profiles by running the GSEA algorithm on the contrast-drug profile correlation space. The activation-heatmap compares drug activation profiles across multiple contrasts. This facilitates to quickly see and detect the similarities between contrasts for certain drugs.
"
...
```{r}
tipify( selectInput("fa_contrast","Contrast:", choices=NULL),
       "Select the contrast corresponding to the comparison of interest.",
       placement="top")
br();br();

##actionLink("fa_options", "Show options ...")
tipify( actionLink("fa_options", "Options", icon=icon("cog", lib = "glyphicon")),
       "Show/hide advanced options", placement="top")
br();br();
conditionalPanel(
    "input.fa_options % 2 == 1",
    tagList(
        checkboxInput('fa_normalize','normalize activation matrix',TRUE)
    )
)
...
if(PRO.VERSION) {
    conditionalPanel(
        "input.fa_options % 2 == 1",
        tagList(
            checkboxInput('fa_filtertable','filter signficant (tables)',FALSE)
            ##checkboxInput('dsea_showall','Show all (CMap)',FALSE)
        )
    )
}
...
kegg_table.RENDER <- reactive({
    ...
    DT::datatable( df, rownames=FALSE,
                  ...
                  options=list(
                      ...
                      scrollY = 210, scroller=TRUE, deferRender=TRUE
                  )  ## end of options.list 
                  ) %>%
...
kegg_actmap.RENDER <- reactive({
    ...
    corrplot( score2, is.corr=FALSE, cl.pos="n", col=BLUERED(100),
             tl.cex=cex2, tl.col="grey20", mar=c(bmar,0,3,0) )
...
kegg_buttons1 = plotModuleButtons(
    "kegg_graph", text=kegg_info1, just.info=TRUE, no.download=TRUE,
    title = "KEGG pathway map", label="a"
)

kegg_actmap.opts = tagList()
kegg_actmap_module <- plotModule(
    "kegg_actmap", kegg_actmap.RENDER,
    title = "Activation matrix", label="c",
    info.text = "The KEGG activation matrix visualizes the activation levels of pathways (or pathway keywords) across multiple contrast profiles. This facilitates to quickly see and detect the similarities of certain pathways between contrasts. The size of the circles correspond to their relative activation, and are colored according to their upregulation (red) or downregulation (blue) in the contrast profile.",
    ##options = kegg_actmap.opts,
    pdf.width=10, pdf.height=10, res=80
)
output$kegg_actmap     <- kegg_actmap_module$render
output$kegg_actmap_pdf <- kegg_actmap_module$pdf
    
kegg_table_module <- tableModule(
    id = "kegg_table", label="b",
    func = kegg_table.RENDER,
    info.text = "<strong>Enrichment table.</strong> The table is interactive; enabling user to sort on different variables and select a pathway by clicking on the row in the table. The scoring is performed by considering the total number of genes in the pathway (n), the number of genes in the pathway supported by the contrast profile (k), the ratio of k/n, and the ratio of |upregulated or downregulated genes|/k. Additionally, the table contains the list of the upregulated and downregulated genes for each pathway and a q value from the Fisher’s test for the overlap.", 
    title = "Enrichment table"
)
##output$kegg_table     <- kegg_table_module$render
##output$kegg_table_csv <- kegg_table_module$csv
output <- attachModule(output, kegg_table_module)

fillRow(flex = c(1.3,0.04,1), 
        fillCol(flex = c(NA,1.8,0.06,NA,1),
                kegg_buttons1,
                imageOutput("kegg_graph", width="100%", height="100%"),
                br(),
                kegg_table_module$buttons,
                dataTableOutput('kegg_table')
                ),
        br(),  ## horizontal space
        fillCol(flex = c(NA,1),
                kegg_actmap_module$buttons,
                plotOutput('kegg_actmap')
                )
        )
...
> **KEGG pathways** are a collection of manually curated pathways representing the current knowledge of molecular interactions, reactions and relation networks as pathway maps. **(a)** In the pathway map, genes are colored according to their upregulation (red) or downregulation (blue) in the contrast profile. **(b)** Each pathway is scored for the selected contrast profile and reported in the table. **(c)** The activation matrix visualizes the activation levels of pathways across multiple contrast profiles. 
...
GO_network.RENDER <- reactive({
    ...
    ## remove root?
    removeroot=TRUE
    ##if(PRO.VERSION) removeroot <-input$GO_removeroot
    if(removeroot) {
    ...
    astree = TRUE
    ##if(PRO.VERSION) astree <- input$GO_astree
    if(astree) {
    ...
    ##if("color clusters" %in% input$GO_options) {
...
GO_network.opts = tagList(
    tipify( checkboxInput("GO_prunetree", "Prune tree", TRUE),
           "Prune the tree with only significant branches."),
    tipify( checkboxInput("GO_colorclusters", "Color clusters"),
...
GO_network_module <- plotModule(
    ...
    info.text = GO_info1, no.download=TRUE,
    options = GO_network.opts,
    pdf.width=10, pdf.height=10, res=80
)
##output$GO_network     <- GO_network_module$render
##output$GO_network_pdf <- GO_network_module$pdf
...
##output$GO_actmap     <- GO_actmap_module$render
##output$GO_actmap_pdf <- GO_actmap_module$pdf
...
##output$GO_table     <- GO_table_module$render
##output$GO_table_csv <- GO_table_module$csv
output <- attachModule(output, GO_table_module)

fillRow(flex = c(1.2,1), 
        fillCol(flex = c(NA,2,NA,1),
                GO_network_module$buttons,
                visNetworkOutput("GO_network", height = "100%"),
                GO_table_module$buttons,
                dataTableOutput('GO_table')
                ),
        fillCol(flex = c(NA,1),
                GO_actmap_module$buttons,                
                plotOutput('GO_actmap')
                )
        )

```

> The **Gene Ontology (GO)** describes our knowledge of the biological domain of genes with respect to three aspects: molecular function, cellular component and biological process. **(a)** Enrichment of the GO terms represented as a hierarchical graph. **(b)** GO scoring table. The score of a GO term is the cumulative score of all higher order terms. **(c)** The activation matrix visualizes the activation levels of GO terms across multiple contrast profiles.
...
dsea_table.RENDER <- reactive({
    ...
    DT::datatable( res, rownames=FALSE,
                  ...
                  options=list(
                      ...
                      dom = 'lfrtip', 
                      scrollX = TRUE, ##scrollY = TRUE,
                      scrollY = 350, scroller=TRUE, deferRender=TRUE
                  )  ## end of options.list 
                  ) %>%
        DT::formatStyle(0, target='row', fontSize='11px', lineHeight='70%') %>% 
            DT::formatStyle( "NES",
...
dsea_enplots.opts = tagList(
    tipify( radioButtons('dsea_monocombo',"Analysis type:",c("mono","combo"),inline=TRUE),
...
##output$dsea_enplots     <- dsea_enplots_module$render
##output$dsea_enplots_pdf <- dsea_enplots_module$pdf
output <- attachModule(output, dsea_enplots_module)


##---------- DSEA Activation map plotting module
dsea_moaplot.opts = tagList(
    tipify( radioButtons('dsea_moatype','Plot type:',c("drug class","target gene"),inline=TRUE),
...
##output$dsea_actmap     <- dsea_actmap_module$render
##output$dsea_actmap_pdf <- dsea_actmap_module$pdf
...
##output$dsea_table     <- dsea_table_module$render
##output$dsea_table_csv <- dsea_table_module$csv
...
## Page layout
##-----------------------------------------
fillRow(flex = c(2.6,1), 
        fillCol(flex = c(1.4,0.07,NA,1),
                fillRow(flex=c(2.2,1),
                        fillCol(flex = c(NA,1),
                                dsea_enplots_module$buttons,
                                plotOutput("dsea_enplots")
                                ),
                        fillCol(flex = c(NA,1),
                                dsea_moaplot_module$buttons,
                                plotOutput("dsea_moaplot")
                                )
                        ),
                br(),  ## vertical space
                dsea_table_module$buttons,
                dataTableOutput('dsea_table')
                ),
        fillCol(flex = c(NA,1),
                dsea_actmap_module$buttons,
                plotOutput('dsea_actmap')
                )
        )
...
> The **Drug Connectivity Map** (Drug CMap) correlates your signature with more than 5000 known drug profiles from the L1000 database. **(a)** The figure shows the top N=10 similar and opposite profiles by running the GSEA algorithm on the contrast-drug profile correlation space. **(b)** The table summarizes the statistical results of the drug enrichment analysis. **(c)** The mechanism-of-action plot shows the top most frequent drug class (or target genes) having similar or opposite enrichment compared to the query signature. **(d)** The activation matrix visualizes the enrichment levels of drug signatures across multiple contrast profiles. 
...
italk_LRPlot.RENDER <- reactive({
    ...
    title((paste(comm_type,"genes     ")), line=-2)
})
...
italk_netview_module <- plotModule(
    ...
    pdf.width=5, pdf.height=8, res=80
)
##output$italk_netview     <- italk_netview_module$render
##output$italk_netview_pdf <- italk_netview_module$pdf
output <- attachModule(output, italk_netview_module)


italk_LRPlot_module <- plotModule(
    "italk_LRPlot", italk_LRPlot.RENDER,
    title = "Ligand-Receptor plot", label="b",
...
a circle plot. The width of the arrow represents the expression level/log fold change of the ligand; while the width of arrow head represents the expression level/log fold change of the receptor. Different color and the type of the arrow stands for whether the ligand and/or receptor are upregulated or downregulated. For further information, see iTALK R package (Wang et al., BioRxiv 2019).",
    options = tagList(
        tipify( selectInput("italk_LRPlot_ntop","ntop pairs",
                            choices=c(10,15,20,25,30,40,50),selected=25),
               "Select the maximum number of LR pairs to include in the LR plot.",
               placement="top")
    ),
    pdf.width=5, pdf.height=8, res=80
)
output$italk_LRPlot     <- italk_LRPlot_module$render
output$italk_LRPlot_pdf <- italk_LRPlot_module$pdf

italk_heatmap_module <- plotModule(
    "italk_heatmap", italk_heatmap.RENDER,
    title = "Expression heatmap", label="c",
...
output$italk_heatmap     <- italk_heatmap_module$render
output$italk_heatmap_pdf <- italk_heatmap_module$pdf
...
fillCol(flex = c(NA,1), 
        ...
        fillRow(flex = c(1,1,1),
                fillCol(flex = c(NA,1),
                        italk_netview_module$buttons,
                        plotOutput('italk_netview')
                        ),
                fillCol(flex = c(NA,1),
                        italk_LRPlot_module$buttons,
                        plotOutput('italk_LRPlot')
                        ),
                fillCol(flex = c(NA,1),
                        italk_heatmap_module$buttons,
                        plotOutput('italk_heatmap')
                        )
        )
        )


##bsTooltip("italk_groups","Select the parameter to divide in groups.", placement="right")
##bsTooltip("italk_category","Select the gene category for finding L/R pairs.", placement="right")

require(tippy)
##tippy_this("italk_groups","Select the parameter to divide in groups.")
##tippy_this("italk_category","Select the gene category for finding L/R pairs.")
...
> **Visualization of ligand-receptor interaction with iTALK.** The iTALK R package is designed to profile and visualize the ligand-receptor mediated intercellular cross-talk signals. iTALK uses a manually curated list of ligand-receptor gene pairs further classified into 4 categories based on the primary function of the ligand: cytokines/chemokines, immune checkpoint genes, growth factors, and others. It was originally designed for single-cell RNA sequencing data (scRNA-seq). For further information see this [iTALK paper](https://www.biorxiv.org/content/10.1101/507871v1). **(a)** The NetView plot visualizes the communication structure of ligand-receptor genes as a graph. **(b)** The Ligand-Receptor plot visualizes the communication structure of ligand-receptor genes as a circle plot. **(c)** The heatmap visualizes the expression level/log fold change of the ligand/receptor genes. 
