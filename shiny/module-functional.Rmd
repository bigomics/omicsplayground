...
if(PRO.VERSION) radioButtons('dsea_label','C-map label',c("name","moa","target"),inline=TRUE)
if(PRO.VERSION) checkboxInput('dsea_showall','C-map show all',FALSE)
...
output$fa_keggtable <- renderDataTable({
    ...
    DT::datatable( df, rownames=FALSE,
        ...
        DT::formatStyle(0, target='row', fontSize='11px', lineHeight='70%') %>% 
            DT::formatStyle( "delta.pct",
                            ##background = styleColorBar(c(0,3), 'lightblue'),
                            background = color_from_middle( df[,"delta.pct"], 'lightblue', '#f5aeae'),
...
observe({
    ...
    updateSelectInput(session, "dsea_monocombo", choices=ct )
})
...
output$fa_italk <- renderPlot({
    ...
    length(genes)
    ##genes <- intersect(genes, rownames(ngs$counts))
    ...
    data <- data.frame(cell_type=ct, t(ngs$X[genes,]))
    ...
    cell_col <- structure(c('#4a84ad','#4a1dc6','#e874bf','#b79eed', '#ff636b', '#52c63b','#9ef49a'),
                          names=unique(as.character(data$cell_type)))
