...
<br><br>For a tutorial on drug enrichment analysis, see the video below:<br><br>
<center><iframe width='500' height='333' src='https://www.youtube.com/embed/WaBVz0a9AVg' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></center>
")
...
fillRow(flex = c(1.3,0.04,1),
        fillCol(flex = c(NA,1.8,0.06,NA,1),
                kegg_buttons1,
                imageOutput("kegg_graph", height="100%"),
                br(),
        ...
        fillCol(flex = c(NA,1),
                ...
                plotOutput('kegg_actmap', height = "100%")
                )
        )
...
> **KEGG pathway analysis.** KEGG pathways are a collection of manually curated pathways representing the current knowledge of molecular interactions, reactions and relation networks as pathway maps. **(a)** Colored KEGG pathway map. Genes are colored according to their upregulation (red) or downregulation (blue) in the contrast profile. **(b)** Table reporting (Fisher's exact) enrichment score for each pathway for the selected contrast profile. **(c)** Activation matrix visualizing activation levels of pathways across multiple contrast profiles. 
...
fillRow(flex = c(1.2,1), 
        ...
        fillCol(flex = c(NA,1),
                ...
                plotOutput('GO_actmap', height = "100%")
                )
        )

```

> **Gene Ontology (GO) analysis.** GO describes our knowledge of the biological domain of genes with respect to three aspects: molecular function, cellular component and biological process. **(a)** Hierarchical graph representing the enrichment of the GO terms as a tree structure. **(b)** GO scoring table. The score of a GO term is the cumulative score of all higher order terms. **(c)** Activation matrix visualizing the enrichment of GO terms across multiple contrast profiles.
...
> **Drug Connectivity Map.** Drug CMap correlates your signature with more than 5000 known drug perturbation profiles from the L1000 database. **(a)** Figure showing the top N=10 similar and opposite profiles by running the GSEA algorithm on the contrast-drug profile correlation space. **(b)** Table summarizing the statistical results of the drug enrichment analysis. **(c)** Mechanism-of-action plot showing the top most frequent drug class (or target genes) having similar or opposite enrichment compared to the query signature. **(d)** Activation matrix visualizing enrichment levels of drug signatures across multiple contrast profiles. 
...
enrich_keyword.RENDER <- reactive({
    ...
    cat("<enrich_keyword> 3: keyword=",keyword,"\n")
...
wordcloud_enrichmentTable.RENDER <- reactive({    
    ...
    req(df)
    df <- df[,c("word","pval","padj","ES","NES","size","tsne.x","tsne.y")]
...
wordcloud_leadingEdgeTable.RENDER <- reactive({    
    ...
    df <- data.frame("leading.edge"=ee, NES=0)    
    numeric.cols <- which(sapply(df, is.numeric))
    numeric.cols
...
wordcloud_actmap.RENDER <- reactive({
    ...
    corrplot( score2, is.corr=FALSE, cl.pos = "n", col=BLUERED(100),
             tl.cex=cex2, tl.col="grey20", mar=c(bmar,0,1,0) )
...
enrich_wordtsne_module <- plotModule(
    ...
    options = tagList(
        radioButtons("enrich_wordtsne_algo","Clustering algorithm:",
...
enrich_wordcloud_opts = tagList(
    selectInput("enrich_wordcloud_exclude","Exclude words:",
...
enrich_wordcloud_module <- plotModule(
    ...
    ##renderFunc="renderWordcloud2",
    plotlib="htmlwidget", renderFunc="renderd3Cloud",
    ##download.fmt = NULL,
    info.text = "<strong>Word cloud.</strong> Word cloud of the most enriched keywords for the data set. Click on the figure to select a keyword for the 'Keyword enrichment analysis' chart. In the plot settings, users can exclude certain words from the figure.<br><br>Size of the words are relative to the normalized enrichment score (NES) from the GSEA computation. Keyword enrichment is computed by running GSEA on the mean (squared) enrichment profile (averaged over all contrasts). For each keyword, we defined the 'keyword set' as the collection of genesets that contain that keyword in the title/description.",
...
enrich_keyword_info = "<strong>Keyword enrichment analysis.</strong> Computes enrichment of a selected keyword across all contrasts. Select a keyword by clicking a word in the 'Word cloud' plot.
...
enrich_keyword_opts = tagList( tipify(
    ##textInput('enrich_keyword_keywords','Keyword:',"cell cycle"),
...
fillRow(
    ...
    fillCol(
        ...
        fillRow(
            ...
            moduleWidget(enrich_keyword_module),
            br(),
            moduleWidget(enrich_wordcloud_module, outputFunc="d3CloudOutput"),
            br(),
            ##moduleWidget(enrich_wordtsne_module), 
            moduleWidget(enrich_wordtsne_module, outputFunc="plotlyOutput")
        ),
        br(),
        fillRow(
            flex=c(1.3,0.08,1),
            moduleWidget(wordcloud_enrichmentTable_module, outputFunc="dataTableOutput"),
            br(),
            moduleWidget(wordcloud_leadingEdgeTable_module, outputFunc="dataTableOutput")
        )
    ),
    moduleWidget(wordcloud_actmap_module)
)


```   

> **Keyword enrichment analysis.** **(c)** Enrichment plots for the top most significant contrasts. Black vertical bars indicate the position of gene sets, in the ranked enrichment scores, that contains the *keyword*. The green curve corresponds to 'running statistics' of the keyword enrichment score. **(b)** Colored word cloud. The size of the words are relative to the normalized enrichment score (NES) from the GSEA computation. **(c)** T-SNE plot of keywords extracted from the titles/descriptions of the genesets. **(d)** Activation matrix showing keyword enrichment across contrasts. **(e)** Keyword enrichment table for selected contrast. **(f)** Leading edge terms for selected keyword enrichment.
