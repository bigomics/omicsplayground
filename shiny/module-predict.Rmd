...
##----------------------------------------------------------------------
##actionLink("pdx_info", "More details ...")
actionLink("pdx_info", "Info", icon = icon("info-circle") )
...
```{r}
selectInput("pdx_contrast","Predicted variable", choices=NULL)
actionButton("pdx_runbutton",label="Run")

br();br();br();
actionLink("pdx_options", "Options", icon=icon("cog", lib = "glyphicon"))
br();br();
conditionalPanel(
...
if(DEV.VERSION) {
    conditionalPanel(
        "input.pdx_options % 2 == 1",
        tagList(
            checkboxInput('pdx_showloops','detect loops',FALSE),
            checkboxInput('pdx_posonly','pos.only',FALSE),
...
observe({
    ...
    req(input$pdx_filter)
    req(input$pdx_contrast)
...
output$pdx_heatmap <- renderPlot({
    ...
    splitx <- NULL
    if(FALSE && input$pdx_vartype=="contrast") {
        ct <- input$pdx_contrast
        exp.matrix <- ngs$model.parameters$exp.matrix
        splitx <- exp.matrix[colnames(X),ct]
    }
    if(input$pdx_vartype=="phenotype") {
        ct <- input$pdx_contrast
