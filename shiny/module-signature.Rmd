...
tipify(selectInput("sig_type",label="Signature type:",choices=c("<custom>","contrast","hallmark","KEGG")),
...
conditionalPanel(
    ...
    tipify(selectizeInput("sig_feature","Signature:", choices="<custom>", selected="<custom>"),
           "Select a specific signature group.", placement="top", options = list(container = "body"))
)
tipify(textAreaInput("sig_genelistUP", "Genes:", value = IMMCHECK.GENES,
...
tipify(actionButton("sig_example2","[apoptosis] ", style="font-size: 0.9em; color: #24A; background-color:
 transparent; border-style: none; padding:0"),
 "Use the list of genes involved in apoptosis as a signature.")
tipify(actionButton("sig_example1","[immune_chkpt] ", style="font-size: 0.9em; color: #24A; background-color: transparent; border-style: none; padding:0"),
       "Use the list of genes involved in immune checkpoint as a signature.")
tipify(actionButton("sig_example3","[cell_cycle] ", style="font-size: 0.9em; color: #24A; background-color: transparent; border-style: none; padding:0"),"Use the list of genes involved in cell cycle as a signature.")
...
sig_enplots.opts = tagList(
    tipify( selectInput('cmp_querydataset',"Query dataset:", choices=NULL, multiple=FALSE),
           "The query dataset to which the enrichment test should be applied. Enrichment of the selected signature will be calculated to all available contrast profiless in this query dataset.", placement = "top")
)
sig_enplots.module <- plotModule(
    "sig_enplots", sig_enplotsFUNC, plotlib="base",
    info.text = "The <strong>Enrichment</strong> tab performs the enrichment analysis of the gene list against all contrasts by running the GSEA algorithm and plots enrichment outputs. Enrichment statistics can be found in the corresponding table",
    options = sig_enplots.opts,
    pdf.width=8, pdf.height=8, res=90
)
output <- attachModule(output, sig_enplots.module)
fillCol( moduleWidget(sig_enplots.module))
...
sig_overlapScorePlot.opts = tagList(
    tipify(radioButtons("sig_overlapScorePlot_ntop","Number of features",c(60,120,250),inline=TRUE),
           "Specify the number to top features to show.", placement="top", options = list(container = "body")),
    tipify(checkboxInput("sig_overlapScorePlot_shownames","Show feature names",TRUE),
...
fillCol(
    flex = c(1,0.04,1), 
    moduleWidget(sig_overlapScorePlot.module, outputFunc="plotlyOutput"),
    br(),
    moduleWidget(sig_overlapTable.module, outputFunc="dataTableOutput")
)

```

> **Overlap/Similarity table.** **(a)** Top overlapping gene sets with
> selected signature. The vertical axis shows the overlap score of the
> gene set which combines the odds ratio and significance (q-value) of
> the Fisher's test. **(b)** Table summarizing the results of the
> Fishers's test for overlap. The column \'common genes\' reports the
...
fillCol( moduleWidget(sig_markers.module) )
...
sig_enrichmentByContrastTable.RENDER <- reactive({
    ...
    DT::datatable(output, class='compact cell-border stripe',
                  ...
                  options=list(
                      ...
                      dom = 'lrtip',
                      ## pageLength = 40, ##lengthMenu = c(20, 30, 40, 60, 100, 250),
                      scrollX = TRUE, scrollY = 280, scroller=TRUE, deferRender=FALSE,
...
> **Enrichment of query signature across all contrasts.** **(a)**
> Enrichment scores across all contrasts for the selected query
> signature . The NES corresponds to the normalized enrichment score
> of the GSEA analysis.  **(b)** Genes in the query signature sorted
> by decreasing (absolute) fold-change corresponding to the contrast
> selected in Table (a).
