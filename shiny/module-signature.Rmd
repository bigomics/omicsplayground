...
<br> **Signature Analysis**. Users can test their gene signature by
calculating an enrichment score. You can use some example genes sets
below or upload your own gene list. Instead of a list of genes, a
contrast can be selected, which then takes the top differentially
expressed genes as signature.
...
##----------------------------------------------------------------------
tipify( actionLink("sig_info", "Info", icon = icon("info-circle")),
...
textAreaInput("sig_genelistUP", "Signature:", value = IMMCHECK.GENES,
...
if(PRO.VERSION || DEV.VERSION) {
    ...
    tipify( actionLink("sig_options", "Options", icon=icon("cog", lib = "glyphicon")),
...
##output$sig_markersplot     <- sig_markers.module$render
##output$sig_markersplot_pdf <- sig_markers.module$pdf
output <- attachModule(output, sig_markers.module)

fillCol(
    flex = c(NA,1), 
    sig_markers.module$buttons,
    plotOutput("sig_markersplot", height = "100%", width = "100%")
)

```

> **Markers t-SNE plot**. This figure shows the t-SNE plot for each gene, where the dot (corresponding to samples) are colored depending on the upregulation (in red) or downregulation (in blue) of that particular gene.
...
sig_enplots.opts = tagList(
    selectInput('cmp_cmapsets1',"Test dataset:", choices=NULL, multiple=FALSE)
)
sig_enplots.module <- plotModule(
    "sig_enplots", sig_enplotsFUNC, plotlib="base",
    info.text = "The <strong>Enrichment</strong> tab performs the enrichment analysis of the gene list against all contrasts by running the GSEA algorithm and plots enrichment outputs. The enrichment statistics can be found in the corresponding table",
    options = sig_enplots.opts,
    pdf.width=8, pdf.height=8, res=90
)
output$sig_enplots     <- sig_enplots.module$render
output$sig_enplots_pdf <- sig_enplots.module$pdf

fillCol(flex = c(NA,1), 
        sig_enplots.module$buttons,
        plotOutput("sig_enplots", height = "100%", width = "100%")
        )

```

> **Enrichment plots.** This figure shows the enrichment of the query signature in all constrasts. Positive enrichment means that this particular contrast shows similar expression changes as the query signature.
...
cmp_cmapplotD3_RENDER <- reactive({
...
## cmp_cmapplotD3_RENDER

cmp_cmapplotD3.opts = tagList(
    selectInput('cmp_cmapsets2',"Test dataset:", choices=NULL, multiple=TRUE),
...
cmp_cmapplotD3.module <- plotModule(
    "cmp_cmapplotD3", cmp_cmapplotD3_RENDER, plotlib="scatterD3",
    info.text = "The Signature Connectivity Map (CMap) associates the provided signature list or contrast profile with the similar profiles of the other experiments, in which we have taken ten datasets from the literature.",
    options = cmp_cmapplotD3.opts,
    pdf.width=8, pdf.height=8, res=90
)
output$cmp_cmapplotD3     <- cmp_cmapplotD3.module$render
output$cmp_cmapplotD3_pdf <- cmp_cmapplotD3.module$pdf

fillCol(flex = c(NA,1), 
        cmp_cmapplotD3.module$buttons,
...
> **Signature Connectivity Map.** This figure shows the enrichment of the query signature in all constrasts. Positive enrichment means that this particular contrast shows similar expression changes as the query signature.
...
sig_overlaptable.module <- plotModule(
    "sig_overlaptable", sig_overlaptable.RENDER, plotlib="table", just.info=TRUE,
    title = "Overlap with other signatures", no.download=TRUE,
...
##DT::renderDataTable
output$sig_overlaptable <- sig_overlaptable.module$render
##output$sig_overlaptable_pdf <- sig_overlaptable.module$pdf

fillCol(flex = c(NA,NA,1), 
        sig_overlaptable.module$buttons, br(),
        dataTableOutput('sig_overlaptable')
        )

```

> **Overlap/Similarity table.** This table...
...
sig_phenoplot.opts = tagList(
    radioButtons('sig_labelmode',"Label",c("groups","legend"),inline=TRUE)
)

sig_phenoplot.module <- plotModule(
    "sig_phenoplot", sig_phenoplot.RENDER, ## plotlib="base",
    info.text = "Phenotypes. This figure visualizes the distribution of the phenotype information.",
    options = sig_phenoplot.opts,
    pdf.width=6, pdf.height=9, res=85
)
output$sig_phenoplot     <- sig_phenoplot.module$render
output$sig_phenoplot_pdf <- sig_phenoplot.module$pdf

fillCol(flex = c(NA,1), 
        sig_phenoplot.module$buttons,
...
> **Phenotype distribution plots.** The plots show the distribution of the phenotypes superposed on the t-SNE clustering. Often, we can expect the t-SNE distribution to be driven by the particular phenotype that is controlled by the experimental condition or unwanted batch effects.
...
sig_table1_module <- tableModule(
    id = "sig_table1", func = sig_table1.RENDER,
    info.text = info.text1,
    title = "Enrichment by contrasts", label="a"
)
output <- attachModule(output, sig_table1_module)


info.text2 = "This table shows the genes of the current signature."
sig_table2_module <- tableModule(
    id = "sig_table2", func = sig_genes1.RENDER,
    info.text = info.text2,
    title = "Genes in signature", label="b"
)
output <- attachModule(output, sig_table2_module)
    
fillCol(
    flex = c(NA,1.2,NA,1), ## width = 600,
    sig_table1_module$buttons,
    dataTableOutput('sig_table1'),
    sig_table2_module$buttons,
    dataTableOutput('sig_table2')
)


```

> **Enrichment tables.** **(a)** Summary of correlation/enrichment of the query signature in all contrasts. **(b)** Summary of fold-changes of genes in the query signature.
