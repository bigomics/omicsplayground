...
<br> **Custom signature**. Test a gene signature by calculating its
...
br();br();br();
if(PRO.VERSION) radioButtons("sig_type",NULL,c("contrast","hallmark"),inline=TRUE)
if(PRO.VERSION) radioButtons('sig_rankmethod','Enrichment method:',c("rcor","fgsea"),inline=TRUE)
##if(PRO.VERSION) radioButtons('sig_clustmethod','Cluster method:',c("tsne","pca"),inline=TRUE)
##if(PRO.VERSION) checkboxInput('sig_invert','invert', FALSE)
##if(PRO.VERSION) radioButtons('sig_top','top:',c(50,100,500),inline=TRUE,selected=100)



if(DEV.VERSION) radioButtons('sig_ssstats','ss-stats:',c("rho","gsva","grp.gsva","rho+gsva","rho+grp.gsva"), inline=TRUE)
```
...
getCurrentMarkers <- reactive({
    ...
    if(input$sig_feature=="<custom>") {
        gset <- input$sig_genelistUP
        if(is.null(input$sig_genelistUP)) return(NULL)    
        if(is.null(gset) || length(gset)==0 || gset=="") return(NULL)
        ##gset <- toupper(gset)
        gset <- strsplit(gset, split="[, \n\t]")[[1]]
        if(length(gset)==1) {
            gene <- sub("^[@#]","",gset[1])
            if(grepl("^@",gset[1]) && gene %in% rownames(ngs$X)) {
                ## most correlated with this gen
                rho <- cor(t(ngs$X), ngs$X[gene,])[,1]
                gset <- head(names(sort(abs(rho),decreasing=TRUE)),36)  ## how many?
            } else {
                ##rx <- paste0("^",toupper(gset[1]))
                rx <- toupper(gset[1])
                gset <- grep(rx, features, value=TRUE)
            }
        }
...
calcSingleSampleValues <- function(X, y, method=c("rho","gsva") ) {
    ...
    if(is.null(names(y)) && length(y)!=nrow(X) ) {
        cat("FATAL ERROR:: calcSingleSampleValues: y must have names if not matched")
        return(NULL)
    }
    ...
    if(sum(y!=0)==0) {
        cat("calcSingleSampleValues:: warning: y is all zero!\n")
    ...
    if("gsva" %in% method) {
        ...
        res.gsva <- GSVA::gsva( X, list("gmt"=gset), method="gsva", parallel.sz=0)
...
getSingleSampleEnrichment <- reactive({
    ...
    cat("getSingleSampleEnrichment:: dim.X=",dim(X),"\n")
    cat("getSingleSampleEnrichment:: dim.gX=",dim(gX),"\n")
    cat("getSingleSampleEnrichment:: len.y=",length(y),"\n")
...
DT::renderDataTable({
    ...
    ##markers <- names(which(ngs$GMT[,"H:HALLMARK_TNFA_SIGNALING_VIA_NFKB"]!=0))
    markers <- getCurrentMarkers()
    if(is.null(markers)) return(NULL)
