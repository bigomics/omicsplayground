...
tipify(selectInput("input_data","", choices=INPUT_FILES, selected=PGX.DEFAULT),
       "Select a dataset for the analysis.", placement="top")
...
if(DEV.VERSION) tipify(actionButton("data_scanfiles",label="rescan file info"), 
                       "Click to rescan newly injected dataset into the platform.", placement="bottom")
...
output$splash <- renderUI({
    ...
    fillCol(flex = c(NA, 0.01, 1),
            pgxfiles_module$button,
            br(),
            dataTableOutput('pgxfiles')
    )
...
pgxfiles_text = "This table contains a general information about all available datasets within the platform. For each dataset, it reports a brief description as well as the total number of samples, genes, gene sets (or pathways), corresponding phenotypes and the collection date."

pgxfiles_module <- tableModule(
    id = "pgxfiles", func = pgxfilesFUNC,
    info.text = pgxfiles_text,
    title="Available datasets"
)

output$pgxfiles     <- pgxfiles_module$render
output$pgxfiles_csv <- pgxfiles_module$csv 

```

> The list of available datasets within the platform for further analysis. 
