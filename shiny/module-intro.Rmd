...
if(DEV.VERSION) {
    ...
    LT_FILES <- grep("-LT.pgx$",INPUT_FILES,value=TRUE)
    XL_FILES <- setdiff(INPUT_FILES,LT_FILES)
    INPUT_FILES <- ifelse((sub("-LT","",LT_FILES) %in% INPUT_FILES), XL_FILES, LT_FILES)
    ##INPUT_FILES <- grep("-LT.pgx$",INPUT_FILES,invert=TRUE,value=TRUE)
} else {
    INPUT_FILES = dir(PGX.DIR, pattern="-LT.pgx$")
}
##INPUT_FILES
...
inputData <- eventReactive( input$loadbutton, {
    ...
    if(DEV.VERSION) {
        ...
        LT_FILES <- grep("-LT.pgx$",INPUT_FILES1,value=TRUE)
        XL_FILES <- setdiff(INPUT_FILES1,LT_FILES)
        has.xl <- (sub("-LT","",LT_FILES) %in% INPUT_FILES1)
        INPUT_FILES1 <- ifelse(has.xl, XL_FILES, LT_FILES)
        ##INPUT_FILES <- grep("-LT.pgx$",INPUT_FILES,invert=TRUE,value=TRUE)
    } else {
        INPUT_FILES1 = dir(PGX.DIR, pattern="-LT.pgx$")
    }
