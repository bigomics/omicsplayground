...
<br><br>The phenotype of interest can be multiple categories (classes) or patient survival data. Instead of choosing a phenotype, users can also specify a particular contrast from the analysis and perform biomarker selection. The platform also provides a heatmap of samples based on identified top features. 
...
tipify(selectInput("pdx_predicted","Predicted target:", choices=NULL),
       ...
       placement="top")
tipify(actionButton("pdx_runbutton",label="Run"),
       "Click to start biomarker computation.", placement="right")

br();br();br();
tipify( actionLink("pdx_options", "Options", icon=icon("cog", lib = "glyphicon")),
...
conditionalPanel(
    ...
    tagList(
        ##radioButtons('pdx_vartype',NULL,c("phenotype","contrast"),inline=TRUE),
        ##checkboxInput('pdx_normalize','normalize',TRUE),
        tipify( selectInput("pdx_level","Level:", choices=c("gene","geneset")),
               "Select feature level: gene or geneset", placement="top"),
        tipify( selectInput("pdx_filter","Filter:", choices=NULL),
               "Select a filter for the gene features.", placement="top"),
        tipify(
...
observe({
    ...
    if(input$pdx_level=="geneset") {
        ...
        ## gene level
        ft <- pgx.getFamilies(ngs,nmin=10,extended=TRUE)
    }
...
calcVariableImportance <- eventReactive( input$pdx_runbutton, {
    ...
    if(is.multiomics) {
        ...
        if(0) {
            ...
            ##R.gene <- exp(apply(log(R), 2, function(x) tapply(x,gene,sum)))
            gene.wt <- apply(R, 2, function(x) tapply(x,gene,sum))
            
            ## mean intertype correlation
            jj <- match(rownames(X0),rownames(ngs$genes))
            g0 <- ngs$genes[jj,"gene_name"]
            X1 <- X0[which(g0 %in% gene),]
            dim(X1)
            g1 <- ngs$genes[match(rownames(X1),rownames(ngs$genes)),"gene_name"]
            
            cor.wt <- tapply( 1:nrow(X1), g1, function(ii) {
                rho <- cor(t(X1[ii,]), use="pairwise")
                diag(rho) <- NA
                mean(abs(rho[upper.tri(rho)]),na.rm=TRUE)
            })            
            ## weighting with 
            cor.wt <- cor.wt[match(gene,names(cor.wt))]
            gene.wt <- gene.wt[match(gene,rownames(gene.wt)),]
            R <- R * gene.wt * as.vector(cor.wt)
        } else {
            rr <- rowMeans(R)
...
pdx_boxplots.RENDER <- reactive({
    ...
    cat("pdx_boxplots:: 1: rn.res.X=",head(rownames(res$X)),"\n")
...
output$pdx_importance     <- pdx_importance_module$render
output$pdx_importance_pdf <- pdx_importance_module$pdf
...
output$pdx_heatmap     <- pdx_heatmap_module$render
output$pdx_heatmap_pdf <- pdx_heatmap_module$pdf
...
output$pdx_decisiontree     <- pdx_decisiontree_module$render
output$pdx_decisiontree_pdf <- pdx_decisiontree_module$pdf
...
pdx_boxplots_module <- plotModule(
    ...
    pdf.width=10, pdf.height=10, res=90
)
output$pdx_boxplots     <- pdx_boxplots_module$render
output$pdx_boxplots_pdf <- pdx_boxplots_module$pdf


fillRow(flex = c(1,0.1,1), 
        fillCol(flex = c(NA,0.7,NA,1),
                pdx_importance_module$buttons,
                plotOutput('pdx_importance'),
                pdx_heatmap_module$buttons,
                plotOutput('pdx_heatmap')
                ),
        br(), ## spacer
        fillCol(flex = c(NA,1,NA,0.8), 
                ##plotOutput('pdx_decisiontree'),
                pdx_decisiontree_module$buttons,
                plotOutput('pdx_decisiontree'),
                pdx_boxplots_module$buttons,
...
mix_plotLoadings.RENDER <- reactive({    
    ...
    showloops=FALSE
    if(PRO.VERSION) showloops <- input$mix_showloops
...
output$mix_plotLoadings     <- mix_plotLoadings_module$render
output$mix_plotLoadings_pdf <- mix_plotLoadings_module$pdf
...
output$mix_hivePlot     <- mix_hivePlot_module$render
output$mix_hivePlot_pdf <- mix_hivePlot_module$pdf
...
output$mix_plotSNF     <- mix_plotSNF_module$render
output$mix_plotSNF_pdf <- mix_plotSNF_module$pdf
...
output$mix_cimDiablo     <- mix_cimDiablo_module$render
output$mix_cimDiablo_pdf <- mix_cimDiablo_module$pdf
