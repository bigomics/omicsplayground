...
observeEvent( input$upload_compute, {
    ...
    confirmSweetAlert(
        ...
        title = "Your data will be uploaded to the server for computation<br>Are you sure?"
    )
})
...
observeEvent( input$myconfirmation, {
    ...
    if(isFALSE(input$myconfirmation)) {
        dbg("INFO user cancelled upload")
        return(NULL)
    }
    ...
    if(FALSE && input$upload_name %in% pgx.files) {
        sendSweetAlert(
            session = session,
            title = "File exists!",
    ...
    sendSweetAlert(
        session = session,
        title = "Ready! You can now analyze your uploaded data set.",
...
uploadStatusTable <- reactive({
    ...
    if(status["contrasts.csv"]=="OK") {
        if( ncol(uploaded_files[["contrasts.csv"]]) > 20 ) {
            status["contrasts.csv"] = "ERROR: maximum 20 contrasts allowed"
        }
    }

    ## check files: maximum samples allowed
    if(status["counts.csv"]=="OK" && status["samples.csv"]=="OK") {
        if( ncol(uploaded_files[["counts.csv"]]) > 100 ) {
            status["counts.csv"]  = "ERROR: max 100 samples allowed"
        }
        if( nrow(uploaded_files[["samples.csv"]]) > 100 ) {
            status["samples.csv"] = "ERROR: max 100 samples allowed"
        }
    }
