...
cartoon_list <- list(
    ...
    list(slogan="Big Friendly Omics", img="big-friendly-omics.jpg"),
...
showStartupModal <- function(once=FALSE) {    
    ...
    showModal(modalDialog(
        id="modal-splash",
        div(img(src = base64enc::dataURI(file="www/splash/bigomics-splash1.png"),
                width="auto", height="500px"),
            ##imageOutput("splash_image", width="auto", height="auto"),
...
observeEvent( input$login_btn, {           
    ...
    if (login.OK) {
        ...
        ##show("login_warning")
        if(LOGIN_AUTHENTICATION=="password") {
            output$login_warning = renderText("Invalid user name or password")
        }
        if(LOGIN_AUTHENTICATION=="register") {
...
##    valueBox( value=nrow(PGXINFO()), subtitle="data sets")
##})
```
...
upload_filetypes = c("text/csv","text/comma-separated-values,text/plain",
                     ".csv")

upload_info = "<h4>User file upload</h4><p>Please prepare the data files in CSV format as listed below. It is important to name the files exactly as shown. The file format must be comma-separated-values (CSV) text. Be sure the dimensions, rownames and column names match for all files. On the left, provide a unique name and description for the data set.<br><br>"
...
fillRow(
    ...
    wellPanel(
        ...
        textAreaInput("upload_description", "Description:", value = NULL,
                      rows=5, placeholder="Describe your data set"),
        conditionalPanel(
            "output.allFilesOK",
...
output$allFilesOK <- reactive({    
    ...
    filled = ( input$upload_name!="" && input$upload_description!="")
    ##all.ok    <- all(sapply(uploaded_files, function(x) x$status=="OK"))
    (all.there && all.ok && filled)
})
...
observeEvent( input$upload_compute, {
    ...
    gx.methods   = c("ttest.welch","ttest.rank","trend.limma")
    gset.methods = c("fisher","gsva","fgsea")
...
uploadFileTable <- reactive({
    ...
    description = c(
        ...
        "Contrast file with samples on rows, conditions as columns."        
    )
