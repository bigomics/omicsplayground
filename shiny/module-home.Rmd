...
usermode_infotip = "Select BASIC or PREMIUM user mode. The BASIC mode should be sufficient for most users. The PREMIUM mode unlocks more algorithms, extra visualization panels and advanced analysis modules for single-cell RNA-seq, cell profiling and biomarker analysis."

userlevels = c("BASIC","PREMIUM")
...
observeEvent(input$main_usermode,
{
    ...
    if(input$main_usermode == "PREMIUM") {
        dbg("observeEvent::main_usermode : switching to EXPERT mode")
...
inputData <- reactive({
    ...
    pgx <- currentPGX()
    if(is.null(pgx)) shinyjs::click("loadbutton")    
    dbg("inputData:: pgx$name=",pgx$name,"\n")
    dbg("inputData:: length(pgx)=",length(pgx),"\n")
...
observeEvent( input$upload_compute, {
    ...
    if(0) {
        ...
        showModal( modalDialog(
            ...
            footer = tagList(
                modalButton("Cancel"),
                actionButton("myconfirmation2","Confirm", icon=NULL)
            )
        ))
    }
})
...
observeEvent( input$savedata, {
    ...
    showModal( modalDialog(
        HTML("<center><b>New feature!</b><br>Saving your data to the cloud will be soon available for Premium users!</center>"),
        title = NULL, size = "s", fade=FALSE
    ))
    return(NULL)
    
    ## -------------------- save PGX file/object
    saving.ok = (USERMODE == "expert")
    if(saving.ok) {
        ...
        showModal( modalDialog(
            "Sorry. Saving only possible in expert mode",
            title = NULL, size = "s", fade=FALSE
        ))
    }
