...
showLoadingModal <- function(pgx) {
    ...
    showModal(modalDialog(
        ...
        fillRow(flex=c(1,NA,1), br(),
                ...
                br()),
        footer = HTML("<center><p>Loading ",pgx,"data...  &nbsp; Please wait</p></center>"),
...
output$upload_contents <- renderTable({
    ...
    ## cat("<uploaded_files> names(input$upload_files)=",names(input$upload_files),"\n")
    ...
    ## if(length(ff)) {
    ##     for(i in 1:length(ff)) uploaded_files[[names(ff)[i]]] <- ff[[i]]
    ## }

    ## ## check files
    ## files.uploaded <- uploaded_files()
    ## files.uploaded <- files.uploaded[match(files.needed,names(files.uploaded))]
    ## is.missing = files.needed %in% names(files.uploaded)
    ## files.nrow = sapply(files.uploaded, nrow)
    ## files.ncol = sapply(files.uploaded, ncol)
    
    ## status <- rep("OK",length(files))
    ## df <- data.frame( file.name=names(files), nrow=files.nrow, ncol=files.ncol,
    ##                  is.missing, status)
