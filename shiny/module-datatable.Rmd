...
##radioButtons('data_sampling','samples:', inline=TRUE, choices=c("grouped","random","head"))
checkboxInput('data_geneinfo','gene summary',FALSE)

br();br()
datatypes <- c("CPM","logCPM")
if(PRO.VERSION) datatypes <- datatypes <- c("counts","CPM","logCPM")
...
output$gx_downloadData <- downloadHandler(
    ...
    content = function(file) {
        ...
        for(k in names(ngs$gx.meta$meta)) {
            ff <- ngs$gx.meta$meta[[k]][,c("meta.fx","meta.q")]
