DataView  {data-orientation=rows}
================================================================================
...
<br> **Data View.** Information and descriptive statistics to quickly 
lookup a gene, check the total counts, or view the data tables.
...
conditionalPanel(
    ...
    tagList(
        ...
        tipify( selectInput('data_groupby','Group by:', choices=NULL),
...
data_genePlots_correlationplot_text=paste0('Barplot of the top positively and negatively correlated genes with the gene selected in the ',dropdown_search_gene,'. Absolute expression levels of genes are colored in the barplot, where the low and high expressions range between the light grey and dark black colors, respectively.')
data_genePlots_averageRankPlot_text=paste0('Ranking of the average expression of the selected gene.')
data_geneInfo_text = paste0('To find out more information from the literature, hyperlinks are provide to connect the selected gene in the  ',dropdown_search_gene,' to public databases, including ', a_OMIM,', ', a_KEGG, ' and ',a_GO,'.')
data_corplot_text = paste0('Top cumulative positively and negatively correlated genes with the gene selected in the ',dropdown_search_gene,', across samples (or cells), in the current dataset as well as in public datasets such as ',a_ImmProt,' and ',a_HPA,'. The correlations of genes are colored by dataset.')
...
MARGINS1 = c(8,3.5,2,1)
##MARGINS1 = c(6.5,3.5,2,1)
...
data_genePlots_averageRankPlot.RENDER <- reactive({
    ...
    par(mar=MARGINS1, mgp=c(2.1,0.8,0))
    ##MARGINS1
    plot( mean.fc, type="h", lwd=0.4, col="#3380CC03", #col="grey80",
...
data_genePlots_averageRankPlot_module <- plotModule(
    id="data_genePlots_averageRankPlot", func=data_genePlots_averageRankPlot.RENDER,
...
data_genePlots_correlationplot_module <- plotModule(
    id="data_genePlots_correlationplot", func=data_genePlots_correlationplot.RENDER,
...
data_genePlots_barplot_module <- plotModule(
    id="data_genePlots_barplot", func=data_genePlots_barplot.RENDER,
    info.text=data_genePlots_barplot_text, pdf.width=6, pdf.height=6,
...
data_genePlots_tsne.RENDER <- reactive({
    ...
    par(mar=c(8,2.2,2,2.2), mgp=c(1,0.1,0))
    jj2 <- order(abs(fc1))
    ...
    dbg("[DataView] grp=",grp)
    dbg("[DataView] length.grp=",length(grp))
    dbg("[DataView] head.grp=",head(grp))
    ...
    dbg("[DataView] length.grp=",length(grp))
    dbg("[DataView] head.grp=",head(grp))
    dbg("[DataView] length.samples=",length(samples))
    dbg("[DataView] dim(pos)=",dim(pos))
...
data_genePlots_tsne_module <- plotModule(
    id="data_genePlots_tsne", func=data_genePlots_tsne.RENDER,
...
data_tissueplot.RENDER <- reactive({
    ...
    par(mar=MARGINS1, mgp=c(1.5,0.5,0))
...
data_tissueplot_module <- plotModule(
    id="data_tissueplot", func=data_tissueplot.RENDER,
...
data_corplot_module <- plotModule(
    id="data_corplot", func=data_corplot.RENDER,
...
data_geneInfo_buttons = plotModuleButtons(
    "data_geneInfo_buttons", just.info=FALSE, no.download=TRUE, label="e", text=data_geneInfo_text,
...
fillCol(
    ...
    fillRow(
        ...
        moduleWidget(data_genePlots_tsne_module),
        moduleWidget(data_genePlots_barplot_module),
        moduleWidget(data_genePlots_correlationplot_module),
        moduleWidget(data_genePlots_averageRankPlot_module)
    ),
    br(),
    fillRow(
        ...
        moduleWidget(data_corplot_module),
        moduleWidget(data_tissueplot_module)
    )
)
...
> **Gene plots.**
> **(a)** t-SNE of samples colored by expression of selected gene.
> **(b)** Abundance/expression of selected gene across groups.
> **(c)** Top correlated genes. Level of grey corresponds to absolute expression of the gene.
> **(d)** Average rank of the selected gene compared to other genes.
> **(e)** Further information about the selected gene from public databases.
> **(f)** Barplot showing cumulative correlation in other datasets.
> **(g)** Tissue expression of selected gene.
...
counts_tab_barplot_module <- plotModule(
    id="counts_tab_barplot", func=counts_tab_barplot.RENDER,
...
counts_tab_boxplot.RENDER <- reactive({
    ...
    print("dim of log2counts")
    print(dim(res$log2counts))
...
counts_tab_boxplot_module <- plotModule(
    id="counts_tab_boxplot", func=counts_tab_boxplot.RENDER,
...
counts_tab_histplot_module <- plotModule(
    id="counts_tab_histplot", func=counts_tab_histplot.RENDER,
...
counts_tab_abundanceplot_module <- plotModule(
    id="counts_tab_abundanceplot", func=counts_tab_abundanceplot.RENDER,
...
counts_tab_average_countplot_module <- plotModule(
    id="counts_tab_average_countplot", func=counts_tab_average_countplot.RENDER,
...
fillCol(
    flex = c(1,1),
    fillRow(
        flex = c(1,1,1), id = "counts_tab_row1",
        moduleWidget(counts_tab_barplot_module),
        moduleWidget(counts_tab_boxplot_module),
        moduleWidget(counts_tab_histplot_module)
    ),
    fillRow(
        flex = c(1,1), id = "counts_tab_row2",
        moduleWidget(counts_tab_abundanceplot_module),
        moduleWidget(counts_tab_average_countplot_module)
    )
)
...
data_rawdataTable.RENDER <- reactive({
    ...
    DT::datatable( x, rownames=FALSE,
                  ...
                  options=list(
                      ...
                      scroller=TRUE, scrollX = TRUE, scrollY = 720, 
                      deferRender=TRUE
                  )  ## end of options.list 
                  ) %>%
...
data_rawdataTable_module <- tableModule(
    id = "data_rawdataTable", func = data_rawdataTable.RENDER,
...
> **Gene table.** The table shows the gene expression values per sample, or average expression values across the groups. The column 'rho' reports the correlation with the gene selected in 'Search gene' in the left side bar.
...
data_sampleTable_module <- tableModule(
    id="data_sampleTable", func=data_sampleTable.RENDER,
...
> **Sample information table** with phenotype information of samples.
...
data_contrastTable_module <- tableModule(
    id="data_contrastTable", func=data_contrastTable.RENDER,
...
fillCol(flex = c(NA,1),
        data_contrastTable_module$button,
        dataTableOutput("data_contrastTable", height = "100%", width = "100%")
        )
...
> **Contrast table** summarizing the contrasts of all comparisons. Non-zero entries '+1' and '-1' correspond to the group of interest and control group, respectively. Zero or empty entries denote samples not use for that comparison.
...
datatable_timings_module <- tableModule(
    id="datatable_timings", func=datatable_timings.RENDER,
...
datatable_objectdims_module <- tableModule(
    id="datatable_objectdims", func=datatable_objectdims.RENDER,
...
datatable_objectsize_module <- tableModule(
    id="datatable_objectsize", func=datatable_objectsize.RENDER,
...
> **Resource information.** Details about the execution times of the methods, dimensions and sizes of objects.
