...
data_genePlots_averageRankPlot.RENDER <- reactive({
    ...
    par(mar=c(8,3.5,2,1), mgp=c(1.5,0.5,0)) #mgp=c(2.1,0.8,0))
...
data_genePlots_correlationplotF <- reactive({
    ...
    par(mar=c(8,3.5,2,1), mgp=c(2.1,0.8,0))
...
data_genePlots_barplot.RENDER <- reactive({
    ...
    if(input$data_groupby != "<ungrouped>") {
        ...
        gx.b3plot( gx, grp, las=3, main=gene, ylab=ylab, ## col=klr0[ii], 
                   ...
                   xlab="", names.cex=cx1, srt=45, col=rgb(0.2,0.5,0.8,0.8)) #col=rgb(0.2,0.5,0.9,0.6)) 
    }  else {
        jj <- 1:length(gx)
...
fillCol( flex = c(1.3,1),
        ...
        fillRow(flex = c(1,1,1,1),
                ...
                fillCol(flex = c(NA,1),
                        data_genePlots_barplot_module$button,
                        plotOutput("data_genePlots_barplot", height = "100%", width = "100%")),
                ...
                fillCol(flex = c(NA,1),
                        data_genePlots_averageRankPlot_module$button,
                        plotOutput("data_genePlots_averageRankPlot", height = "100%", width = "100%"))

                ),
        
        fillRow(flex = c(1,1.4,1.6),
...
counts_tab_histplot.RENDER <- reactive({
    ...
    gx.hist <- function(gx, n=1000, main="",ylim=NULL) {
        ...
        h0 <- hist(as.vector(c(gx[jj],min(gx),max(gx))),
                   ...
                   cex.names=res$cx1+0.1, cex.lab=1)
        i = 1
        for(i in 1:ncol(gx)) {
