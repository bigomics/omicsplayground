...
hm_parcoord_text = tagsub("Under the <strong>Parallel Coordinates</strong> panel...")
...
hm_parcoord.RENDER <- reactive({
    ...
    annot = filt$grp.annot
    clust.id <- filt$clust    
    ##zx <- read.csv("https://raw.githubusercontent.com/bcdunbar/datasets/master/parcoords_data.csv")

    jj <- order(colnames(zx))
    zx <- zx[,jj]
...
hm_parcoord_module <- plotModule(
    ...
    plotlib="base", renderFunc="renderPlot",
    download.fmt = c("pdf","html"),
    info.text = hm_splitmap_text,
    options = hm_splitmap_opts,
    pdf.width=10, pdf.height=8, info.width="350px",
...
fillCol(
    moduleWidget(hm_parcoord_module, outputFunc="uiOutput")
)
```

> **Parallel Coordinates.** Heatmap showing gene expression sorted by
> 2-way hierarchical clustering. Red corresponds to overexpression,
> blue to underexpression of the gene. At the same time, gene clusters
> are functionally annotated in the 'Annotate clusters' panel on the
> right.
