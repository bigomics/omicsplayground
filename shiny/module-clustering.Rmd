...
hm_splitmap_opts = tagList(
    ...
    tipify( selectInput('hm_topmode','Top mode:',c("specific","sd","pca"), width='100%'),
           ...
           placement="right", options = list(container = "body")),
    tipify( radioButtons('hm_ntop','Top N:',c(50,150,500),inline=TRUE,selected=50),
           "Select the number of top features to be shown in the heatmap.", placement="top"),
    tipify( radioButtons(
...
hm_getClusterPositions <- reactive({
    ...
    ntop = 1000
    ntop = input$hm_ntop2
    zx = head(zx[order(-apply(zx,1,sd)),],ntop)  ## OK?
    ...
    pos = NULL
    if(input$hm_clustmethod=="default") {
        require(Rtsne)
        force.compute = FALSE
        ## force.compute = TRUE
    ...
    } else if(input$hm_clustmethod=="tsne") {
        cat("getClusterPositions:: computing tSNE...\n")
        showNotification("computing tSNE...\n")
        perplexity = max(min(30,ncol(zx)/4),1)
        perplexity = min((ncol(zx)-1)/3, 30)
...
hm_PCAplot_opts = tagList(
    ...
    tipify( radioButtons('hm_clustmethod',"Layout:",c("default","tsne","pca","umap"),inline=TRUE),
            "Choose the layout method for clustering to visualise.", placement="top"),
    tipify( radioButtons('hm_ntop2',"Ntop:",c(100,1000,4000,9999),inline=TRUE,selected=1000),
            "Number of top genes for dimensionality reduction.", placement="top"),
...
hm_parcoord.RENDER <- reactive({
    ...
    req(filt)
    if(0) {
        zx <- t(apply(ngs$X,1,function(x) tapply(x,ngs$samples$group,mean)))
        zx <- head(zx[order(-apply(zx,1,sd)),],400)
        clust.id <- cutree(hclust(as.dist(1-cor(t(zx)))),5)
    ...
    clust.id <- as.integer(factor(filt$clust))
    ##clust.id <- factor(filt$clust)
    ##clust.id <- filt$clust
    table(clust.id)
...
observeEvent( event_data("plotly_restyle", source = "pcoords"), {
    ...
    dimension_name <- colnames(df)[[dimension + 1]]
...
hm_parcoord.selected <- reactive({
    ...
    req(filt)
    df <- filt$grp.mat
    if(input$hm_pcscale) { df <- t(scale(t(df))) }
    keep <- TRUE
...
hm_parcoord_opts = tagList(
    tipify( selectInput("hm_pcvar", "Variable:", choices="group", width='100%'),
           "Split the parallel axis by this phenotype.",
           placement="right",options = list(container = "body")),
    tipify( checkboxInput('hm_pcscale','Scale values',TRUE),
...
hm_parcoord_module <- plotModule(
    "hm_parcoord", hm_parcoord.RENDER,
...
hm_parcoord_table.RENDER <- reactive({
    ...
    df = hm_parcoord.selected()
    cat("[hm_parcoord_table.RENDER] dim(f)=",dim(df),"\n")
    numeric.cols <- 1:ncol(df)
    DT::datatable(
...
hm_parcoord_table_module <- tableModule(
    id = "parcoord_table", func = hm_parcoord_table.RENDER,
...
fillCol(
    flex = c(1.7,0.05,1),
    moduleWidget(hm_parcoord_module, outputFunc="plotlyOutput"),
    br(),
    moduleWidget(hm_parcoord_table_module, outputFunc="dataTableOutput")
)
```

> **Parallel Coordinates plot.** The Parallel Coordinates plot
> displays the expression levels of selected genes across all
> conditions. On the x-axis the experimental conditions are
> plotted. The y-axis shows the expression level of the genes grouped
> by condition. The colors correspond to the gene groups as defined by
> the hierarchical clustered heatmap.
...
hm2_parcoord.RENDER <- reactive({
    ...
    ## annot = filt$grp.annot
    clust.id <- (filt$clust)    
    if(input$hm2_pcscale) { zx <- t(scale(t(zx))) }

    klrpal = rep(brewer.pal(8,"Set2"),99)
    
    ## build dimensions
    library(parcoords)
    df <- data.frame(gene_group=clust.id, zx)
    parcoords(
        df
       ,rownames = FALSE
       ,reorderable = TRUE
       ...
       ,alphaOnBrushed = 0.15
       ,queue = TRUE
       ,margin = list(top=30,bottom=20,left=50,right=20)
       ##,bundleDimension = "clust"
       ##,bundlingStrength = 0.01
       ##,smoothness = 0.02
       ,color= list(
             colorBy = "gene_group"
...
hm2_parcoord.selected <- reactive({

    filt <- hm_filtered_matrix()
    req(filt)
    df <- filt$grp.mat
    if(input$hm2_pcscale) { df <- t(scale(t(df))) }

    ids <- rownames(df) %in% input$hm2_parcoord_brushed_row_names
    df[ids,,drop=FALSE]
})

hm2_parcoord_opts = tagList(
    tipify( selectInput("hm2_pcvar", "Variable:", choices="group", width='100%'),
           "Split the parallel axis by this phenotype.",
           placement="right",options = list(container = "body")),
    tipify( checkboxInput('hm2_pcscale','Scale values',TRUE),
           "Scale expression values to mean=0 and SD=1.",
           placement="right",options = list(container = "body"))
)


hm2_parcoord_text = tagsub("The <strong>Parallel Coordinates</strong> panel
displays the expression levels of selected genes across all conditions in the analysis. On the x-axis the experimental conditions are plotted. The y-axis shows the expression level of the genes grouped by condition. The colors correspond to the gene groups as defined by the hierarchical clustered heatmap.")

hm2_parcoord_module <- plotModule(
    "hm2_parcoord", hm2_parcoord.RENDER,
    plotlib="htmlwidget", renderFunc="renderParcoords",
    download.fmt = c("png","pdf","html"),
    info.text = hm2_parcoord_text,
    options = hm2_parcoord_opts,
    pdf.width=10, pdf.height=6, info.width="350px",
    title="Parallel coordinates"
)
output <- attachModule(output, hm2_parcoord_module)

hm2_parcoord_table.RENDER <- reactive({

    df = hm2_parcoord.selected()
    cat("[hm_parcoord_table.RENDER] dim(f)=",dim(df),"\n")
    numeric.cols <- 1:ncol(df)
    DT::datatable(
            df, rownames=TRUE, ## escape = c(-1,-2),
...
hm2_parcoord_table_info = "In this table, users can check mean expression values of features across the conditions for the selected genes."

hm2_parcoord_table_module <- tableModule(
    id = "parcoord2_table", func = hm2_parcoord_table.RENDER,
    info.text = hm2_parcoord_table_info,
    title="Selected genes", label="b"
)
output <- attachModule(output, hm2_parcoord_table_module)

fillCol(
    flex = c(1.7,0.05,1),
    moduleWidget(hm2_parcoord_module, outputFunc="parcoordsOutput"),
    br(),
    moduleWidget(hm2_parcoord_table_module, outputFunc="dataTableOutput")
)
```
