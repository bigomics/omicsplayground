...
hm_getClusterPositions <- reactive({
    ...
    if(input$hm_clustmethod=="fixed") {
        ...
        if(!force.compute && pdim==2 && !is.null(ngs$tsne2d) ) {
            ...
            cat("getClusterPositions:: computing 2D tSNE...\n")
        ...
        nnb  = min(min(30,max(3,ncol(zx)/ngrp)),ncol(zx))
        nnb  = max(1,min((ncol(zx)-1)/3, 30))	
        nnb
        pos = umap(
            t(zx),
            n_neighbours = nnb,
...
hm_PCAplot_opts = tagList(
    ...
    tipify( radioButtons('hm_clustmethod',"Layout:",c("fixed","tsne","pca","umap"),inline=TRUE),
...
hm2_parcoord.selected <- reactive({
    pc <- hm2_parcoord.matrix()
    req(pc)
    ids <- rownames(pc$mat) %in% input$hm2_parcoord_brushed_row_names
    pc$mat[ids,,drop=FALSE]
})
