...
##selectInput("hm_splitx","Split by:", choices=NULL, multiple=FALSE)

radioButtons('hm_top','top:',c(50,500),inline=TRUE,selected=50)
##radioButtons('hm_topmode','top.mode',c("sd","biomarker","pca"),inline=TRUE)
checkboxInput('hm_group','grouped',FALSE)

br();br();br();
##if(PRO.VERSION) textAreaInput("hm_customlist","Custom gene list", rows=4)
code.textInput
if(PRO.VERSION) myTextInput("hm_customlist","Custom gene list")            
if(PRO.VERSION) radioButtons('hm_gsetmatrix','Gene set matrix:', choices=c("meta"), inline=TRUE)
##if(PRO.VERSION) radioButtons('hm_updown','Up/down:', choices=c("all","up","down"), inline=TRUE)
##if(PRO.VERSION) checkboxGroupInput('hm_filter','Filter:', choices=c("up","down"), inline=TRUE)
...
fillCol(height = "100%", flex = c(NA, 1), 
        ...
        ##flowLayout( 
            ...
            checkboxGroupInput('hm_options',NULL,choices=c('rowscale','softmax'),
...
output$hm_splitmap <- renderPlot({
    ...
    scale.mode = "row"
    scale.mode = ifelse("rowscale" %in% input$hm_options,"row","none")
    ...
    totnchar
    if(!is.null(splitx) & totnchar > 44) crot=90
...
hm_GetClusterPositions <- reactive({
    ...
    if(!PRO.VERSION || "rowscale" %in% input$hm_options)  zx = t(scale(t(zx)))  ## scale??
