...
hm_filtered_matrix <- reactive({
    ...
    if(input$hm_level=="geneset") {
        ...
        dbg("[clustering] hm_filtered_matrix :: head.hm_customlist=",head(input$hm_customlist))
        if(input$hm_customlist!="") {
            ...
            ##gg1 = intersect(gg1,ngs$genes$gene_name)  ## does not work with mouse!
            gg1 = gg1[which(gg1 %in% toupper(ngs$genes$gene_name))]  ## ok with mouse!          
    ...
    dbg("[clustering] hm_filtered_matrix :: dim(zx)=",dim(zx))
    dbg("[clustering] hm_filtered_matrix :: sum.is.na(zx)=",sum(is.na(zx)))
    
    ## ------------- cluster the genes???
    if(is.null(idx)) {
        dd = as.dist(1 - cor(t(zx),use="pairwise"))
        dd[is.na(dd)] = 1  ## 
...
hm_getClusterPositions <- reactive({
    ...
    if(input$hm_clustmethod=="tsne") {
        ...
        pos = umap(
            t(zx),
            ## n_neighbours = min(15,max(5,ncol(zx)/ngrp)),
            n_components = pdim,
            metric = "euclidean",
            n_components = pdim
        )$layout
