...
```{r}
htmlOutput("curdata3", class="current-data")
##----------------------------------------------------------------------
...
<br><br>For a tutorial on Hierarchical Clustering, see the video below:<br><br>
<center><iframe width="500" height="333" src="https://www.youtube.com/embed/7xHsRkOdVwo" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></center>
...
if(DEV.VERSION) {
    tagList(
        ...
        tipify( radioButtons('hm_gsetmatrix','Gene set matrix (dev):',
                             choices=c("meta"), inline=TRUE),
               "Specify the geneset matrix type to be used in the analysis.",
               placement="bottom")
    )
}
...
hm_filtered_matrix <- reactive({
    ...
    cat("<module_clustering::hm_filtered_matrix> 1\n")
    ...
    cat("<module_clustering::hm_filtered_matrix> 2\n")
    ...
    cat("<module_clustering::hm_filtered_matrix> 3\n")
    ...
    cat("<module_clustering::hm_filtered_matrix> splitx=",splitx,"\n")
    cat("<module_clustering::hm_filtered_matrix> topmode=",topmode,"\n")

    if(splitx=="<none>" && topmode=="specific") topmode <- "sd"

    cat("<module_clustering::hm_filtered_matrix> 4\n")
    ...
    cat("<module_clustering::hm_filtered_matrix> 5\n")
    ...
    cat("<module_clustering::hm_filtered_matrix> done\n")
...
hm1_splitmap.RENDER <- reactive({
    ...
    show_colnames <- input$hm_collabel
    ...
    nrownames = ifelse(input$hm_rowlabel,80,0)
...
hm2_splitmap.RENDER <- reactive({
    ...
    plt <- NULL
    cat("<module-clustering:hm2_splitmap.RENDER> split.var=",split.var,"\n")
    ...
    withProgress(
        message='rendering iHeatmap...', value=0.66,
        {
            ...
            cat("<module-clustering:hm2_splitmap.RENDER> idx=",idx,"\n")
            ...
            cat("<module-clustering:hm2_splitmap.RENDER> head(splitx)=",head(splitx),"\n")
            cat("<module-clustering:hm2_splitmap.RENDER> dim(annot)=",dim(annot),"\n")
            ...
            cat("<module-clustering:hm2_splitmap.RENDER> dim(annotF)=",dim(annotF),"\n")
            cat("<module-clustering:hm2_splitmap.RENDER> dim(X)=",dim(X),"\n")
            
            cex=10
            cex = ifelse(input$hm_rowlabel,10,0)
            ...
            plt <- pgx.splitHeatmapX(
                ...
                row_annot_width=0.03, label_size=cex )
        }
    )
    cat("<module-clustering:hm2_splitmap.RENDER> done\n")
    cat("<module-clustering:hm2_splitmap.RENDER> class(plt)=",class(plt),"\n")
    return(plt)
})


hm_splitmap_opts = tagList(
    tipify( radioButtons("hm_plottype", "Plot type:", choices=c("iHeatmap","ComplexHeatmap"),
                         selected="iHeatmap", inline=TRUE, width='100%'),
           "Choose plot type: iHeatmap (interactive) or ComplexHeatmap (static)",
    ...
    tipify( selectInput('hm_topmode','top mode:',c("specific","sd","pca"), width='100%'),
           "Specify the criteria for selecting features to be used in the heatmap clustering analysis.",
           placement="right", options = list(container = "body")),
    ...
    tipify( radioButtons(
        ...
        placement="bottom"),
    tipify( checkboxInput("hm_rowlabel","Show row labels",TRUE),
           "Show or hide row names."),
    tipify( checkboxInput("hm_collabel","Show column labels",TRUE),
...
hm_splitmap_caption <- reactive({
    text1 = "Heatmap showing gene expression sorted by 2-way hierarchical clustering. Red corresponds to overexpression, blue to underexpression of the gene. At the same time, gene clusters are functionally annotated in the 'Annotate clusters' tab on the right."
    return(text1)
})
...
hm_splitmap_module <- plotModule(
    ...
    pdf.width=10, pdf.height=10, info.width="350px",
    title="Heatmap"
)
...
output$hm1_splitmap  <- renderPlot({
    ...
    dev.print(pdf, file=PDFFILE, width=10, height=10)    
    write("HTML export error",file=HTMLFILE)
}, res=90)
...
output$hm2_splitmap <- renderIheatmap({
    ...
    HTMLFILE = hm_splitmap_module$.tmpfile["html"]  ## from above!
    PDFFILE  = hm_splitmap_module$.tmpfile["pdf"]   ## from above!
    if(file.exists(HTMLFILE)) unlink(HTMLFILE)
    if(file.exists(PDFFILE))  unlink(PDFFILE)
    ##webshot::install_phantomjs()
    ##if(do.png) save_iheatmap(p, vwidth=pdf.width*80,vheight=pdf.height*80,PNGFILE,zoom=2)
    if(!is.null(PDFFILE))  save_iheatmap(p, filename=PDFFILE,  vwidth=10*80, vheight=10*80)
    if(!is.null(HTMLFILE)) save_iheatmap(p, filename=HTMLFILE)
    p
})
...
<br><br>Users can customise the PCA/tSNE plot in the <i>Settings</i>, including the {color} and {shape} of points using a phenotype class, choose t-SNE or PCA layout, label the points, or display 2D and 3D visualisation of the PCA/tSNE plot.'))
...
hm_PCAplot_opts = tagList(
    ...
    tipify( radioButtons('hm_clustmethod',"layout:",c("tsne","pca"),inline=TRUE),
            "Choose the output of the tSNE or PCA to visualise.", placement="top"),
    tipify( checkboxGroupInput('hmpca_options',"Other:", choices=c('label','3D'), inline=TRUE),
...
clustannot_correlation <- reactive({
    ...
    dbg("<clustering:clustannot_plots> reacted\n")
    ...
    dbg("<clustering:clustannot_plots> 1\n")
    ...
    if(is.null(ref)) {
        cat("<clustering:clustannot_plots> WARNING:: ref error\n")
        return(NULL)
    }
    ...
    dbg("<clustering:clustannot_plots> computing correlation\n")
    i=idx[1]
    idxx = setdiff(idx, c(NA," ","   "))
    cat("<clustering:clustannot_plots> idxx=",idxx,"\n")
    cat("<clustering:clustannot_plots> dim(zx)=",dim(zx),"\n")
    cat("<clustering:clustannot_plots> samples=",samples,"\n")
    ...
    for(i in 1:length(idxx)) {
        ...
        cat("<clustering:clustannot_plots> head(gg)=",head(gg),"\n")
    ...
    dbg("<clustering:clustannot_plots> done\n")
    
    ##cat("<clustering:clustannot_plots> done\n")
...
clustannot_plots.PLOTLY <- reactive({
    ...
    for(i in 1:min(9,ncol(rho))) {
        ...
        anntitle <- function(tt) {
            list(text=tt, font = list(size=14),
                 xref="paper", yref="paper",
                 yanchor = "bottom", xanchor = "center",
                 align = "center", x=0.5, y=0.97 , showarrow = FALSE )
        }
...
clustannot_table_info_text = "In this table, users can check mean correlation values of features in the clusters with respect to the annotation references database selected in the <i>Settings</i>."
