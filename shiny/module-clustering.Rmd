...
hm1_splitmap.RENDER <- reactive({
    ...
    show_colnames <- ifelse(ncol(zx) < 100, show_colnames, FALSE)
...
output$hm1_splitmap  <- renderPlot({
    ...
    dbg("renderIheatmap:: exporting hm1 to PDF...")
...
output$hm2_splitmap <- renderIheatmap({
    ...
    dbg("renderIheatmap:: exporting hm2 to PDF and HTML...")
    HTMLFILE = hm_splitmap_module$.tmpfile["html"]  ## from above!
    PDFFILE  = hm_splitmap_module$.tmpfile["pdf"]   ## from above!
    ...
    if(!is.null(PDFFILE) && !is.na(PDFFILE)) {
        save_iheatmap(p, filename=PDFFILE,  vwidth=10*80, vheight=10*80)
    }
    if(!is.null(HTMLFILE) && !is.na(HTMLFILE)) {
