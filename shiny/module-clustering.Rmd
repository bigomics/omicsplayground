...
observe({
    ...
    sel <- c(grep("cell.type|cell.family|type|family",cvar,value=TRUE),"<none>")[1]
...
hm1_splitmap.RENDER <- reactive({
    ...
    show_colnames <- ("col" %in% input$hm_collabel)
    ...
    nrownames = ifelse("row" %in% input$hm_showlabel,80,0)
...
hm2_splitmap.RENDER <- reactive({
    ...
    withProgress(
        message='rendering iHeatmap...', value=0.66,
        {
            ...
            cex=10
            cex = ifelse("row" %in% input$hm_showlabel,10,0)
...
clustannot_plots.PLOTLY <- reactive({
    ...
    NTERMS = 6
    NTERMS = 8
    if(ncol(rho)>6)  NTERMS=6
    if(ncol(rho)<=3) NTERMS=20
    ...
    for(i in 1:min(9,ncol(rho))) {
        ...
        anntitle <- function(tt) {
            list(text=tt, font = list(size=13),
                 ...
                 align = "center", x=0.5, y=0.98 , showarrow = FALSE )
        }
