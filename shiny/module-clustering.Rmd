...
##----------------------------------------------------------------------
tipify( actionLink("clustering_info", "Info", icon = icon("info-circle")),
       "Show more information about this module.")

dropdown_level='<code>Level</code>'
dropdown_features='<code>Features</code>'
menu_options='<code>Options</code>'
opt_filter='<code>Filter samples</code>'
opt_top='<code>top</code>'
opt_grouped='<code>grouped</code>'
opt_custom_genelist='<code>Custom gene list</code>'

GSVA="<a href='https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-14-7'> GSVA</a>"
ssGSEA="<a href='https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-14-7'> ssGSEA</a>"


clustering_infotext = paste0('The <strong>cluster analysis</strong> module performs unsupervised clustering analysis of the samples. The main output of this feature is twofold: i) It generates an interactive heatmap of samples and ii) It also provides a dimensionality reduction plot of samples obtained by PCA or tSNE algorithms. 
<br><br> A clustering analysis can be performed on gene level or gene set level, in which users can specify it in the ',dropdown_level,' dropdown list. For the gene set level analysis, an average expression is computed for each gene set (or pathway) from the gene expression data using summary methods such as ',GSVA,' and ',ssGSEA,'. Users can also select a specific gene family or gene set collection in the ',dropdown_features,'dropdown menu for the analysis.
<br><br> During the visual analysis, users can choose multiple options from the ',menu_options,'. They can filter out some samples in the ',opt_filter,', choose top <em>N</em>={50,500} features in the ',opt_top,' collapse the samples by clicking ',menu_grouped,', and insert a custom gene list in ',opt_custom_genelist,'.')

observeEvent( input$clustering_info, {
    showModal(modalDialog(
        title = HTML("<strong>Clustering Module</strong>"),
        HTML(clustering_infotext),
        easyClose = TRUE ))
})
```
...
conditionalPanel(
    ...
    tagList(
        ...
        tipify( radioButtons('hm_top','top:',c(50,500),inline=TRUE,selected=50),
                "Select the number of top features to be used in the analysis.", placement="top"),
        tipify( checkboxInput('hm_group','grouped',FALSE),
                "Group the samples by a predefined phenotype class.", placement="top")
    )
)
...
if(PRO.VERSION) {
    ...
    conditionalPanel(
        "input.hm_options % 2 == 1",
        tagList(
            #myTextInput("hm_customlist","Custom gene list")
            tipify( textAreaInput("hm_customlist", "Custom feature list:", value = NULL,
                                  rows=3, placeholder="Paste your gene list"),
                    "Paste a custom list to be used as features.", placement="bottom")
        )
    )
}
...
if(DEV.VERSION) {
    ...
    conditionalPanel(
        "input.hm_options % 2 == 1",
        tagList(
            tipify( radioButtons('hm_gsetmatrix','Gene set matrix:', choices=c("meta"), inline=TRUE),
                    "Specify the geneset matrix type to be used in the analysis.", placement="bottom")
        )
    )
}
...
##----------------------------------------------------------------------
##                     Info messsages for Plots
##----------------------------------------------------------------------

hm_splitmap_text = paste0('In the <strong>heatmap</strong> tab, a hierarchical clustering can be performed on gene level or gene set level expression in which users have to specify it under the ',dropdown_level,' dropdown list. 
<br><br>During the heatmap generation, users have various options that they can select in the <code>Options</code>, such as splitting the samples by a phenotype class (e.g., tissue, cell type, or gender) in the <code>split by</code>. In addition, users can specify the top N = {50, 500} features to be used in the heatmap for hierarchical clustering. The criteria to select the top features are: 1) sd - features with the highest standard deviation across all the samples, 2) biomarker - features that are overexpressed in each phenotype class compared to the rest, or by 3) pca - principal components. The top feature selection is specified under the <code>top mode</code> section. For more convenient visualisation of heatmap coloring, users can also scale the row values or normalize them between 0 and 1 by clicking the <code>rowscale</code> and <code>softmax</code>, respectively.')
...
hm_splitmap_opts = tagList(
    tipify( selectInput("hm_splitx", "split by:", choices=NULL, width='100%'),
            "Split the samples by a predetermined phenotype class (e.g. tissue or cell type).", placement="right",options = list(container = "body")),
    tipify( selectInput('hm_topmode','top mode',c("sd","biomarker","pca"), width='100%'),
            "Specify the criteria for selecting features to be used in the heatmap clustering analysis.", placement="right",options = list(container = "body")),
    # fillRow( flex=c(1,1),
    #          selectInput("hm_splitx", "split by:", choices=NULL, width='100%'),
    #          selectInput('hm_topmode','top mode',c("sd","biomarker","pca"), width='100%')),
    tipify( checkboxGroupInput('hm_options2',NULL,choices=c('rowscale','softmax'),
                               selected=c('rowscale','softmax'),inline=TRUE),
            "Scale the row values or normalize them between (0, 1) for more convenient coloring of the heatmap.", placement="bottom")
    
)

hm_splitmap_module <- plotModule(
    "hm_splitmapplot", hm_splitmapFUNC,
    info.text = hm_splitmap_text,
    options = hm_splitmap_opts,
    pdf.width=10, pdf.height=20,
    title="Heatmap"
)

output$hm_splitmapplot     <- hm_splitmap_module$render
output$hm_splitmapplot_pdf <- hm_splitmap_module$pdf

fillCol(flex = c(NA,1), 
        ##actionLink("hm_splitmapplot", label="",icon = icon("gear")),
        hm_splitmap_module$button,
        plotOutput("hm_splitmapplot", height = "100%", width = "100%")
        )

```

> A heatmap of samples obtained by the hierarchical clustering using the top features specified by the user.
...
PCA="<a href='https://www.ncbi.nlm.nih.gov/pubmed/19377034'> PCA</a>"
tSNE="<a href='http://jmlr.org/papers/volume15/vandermaaten14a/vandermaaten14a.pdf'> tSNE</a>"

hm_PCAplot_text = paste0(' The <b>PCA/tSNE</b> tab provides the plot obtained by the principal components analysis (',PCA,') or t-distributed stochastic embedding (',tSNE,') algorithms. This plot shows the relationship (or similarity) between the samples for visual analytics, where similarity is visualized as proximity of the points. Samples that are ‘similar’ will be placed close to each other. 
<br><br>Users can customise the PCA/tSNE plot in the <code>Options</code>, including the <code>color</code> and <code>shape</code> of points using a phenotype class provided in the data, choose the <code>tsne</code> or <code>pca</code> output type, specify the <code>lable</code> of the points or display 2D and <code>3D</code> visualisation of the PCA/tSNE plot.')
...
hm_PCAplot_opts = tagList(
    ...
    br(),
    tipify( radioButtons('hm_clustmethod',label=NULL,c("tsne","pca"),inline=TRUE),
            "Choose the output of the tSNE or PCA to visualise.", placement="top"),
    tipify( checkboxGroupInput('hmpca_options',NULL, choices=c('label','3D'), inline=TRUE),
            "Display the labels of the samples in the plot. Select the 2D or 3D visualisation of the plot.", placement="bottom")
)
...
output$hm_PCAplot     <- hm_PCAplot_module$render
output$hm_PCAplot_pdf <- hm_PCAplot_module$pdf

fillCol(flex = c(NA,1),
        ##actionLink("hm_PCAplot", label="",icon = icon("gear")),
        hm_PCAplot_module$button,
        plotlyOutput("hm_PCAplot", height = "100%", width = "100%")
        )

```

> The PCA/tSNE plot of samples using the top features specified by the user.
...
MSigDB="<a href='http://software.broadinstitute.org/gsea/msigdb'> GO</a>"

clustannot_plots_text = paste0('The top features of the heatmap under the <code>Heatmap</code> section are then divided into five clusters based on their expression profile patterns. For each cluster, the platform provides a functional annotation under the <code>Annotate cluster</code> section by correlating more than 42 published reference databases, including but not limited to well-known databases such as ',MSigDB,', ',KEGG,' and ',GO,'. In the <code>Options</code>, users can specify the level and collection of references to be used in the annotation under the <code>Reference level</code> and <code>Reference set</code>, respectively.')
...
clustannot_plots_opts = tagList(
    tipify( selectInput("xann.level", "Reference level:",
            ...
            "Select the level of an anotation analysis.", placement="left", options = list(container = "body")),
    tipify( selectInput("xann.refset", "Reference set:", choices="", width='80%'),
            "Specify a reference set to be used in the annotation.", placement="left",options = list(container = "body"))
)

clustannot_plots_module <- plotModule(
    id="clustannot_plots", func=clustannot_plotsFUNC,
    plotlib = "base", info.text = clustannot_plots_text,
    options = clustannot_plots_opts,
    pdf.width=15, pdf.height=10, res=80,
    title="Functional annotation of clusters"
)

output$clustannot_plots     <- clustannot_plots_module$render
output$clustannot_plots_pdf <- clustannot_plots_module$pdf

    
clustannot_tableFUNC <- reactive({
    rho = clustannot_correlation()
    if(is.null(rho)) return(NULL)
...
clustannot_table_info_text = "In this table, users can check mean correlation values of features in the clusters with respect to the annotation references database selected in the <code>Options</code>."

clustannot_table_module <- tableModule(
    id = "clustannot_table", func = clustannot_tableFUNC,
    info.text = clustannot_table_info_text,
    title="Annotation scores"
)

output$clustannot_table <- clustannot_table_module$render
output$clustannot_table_csv <- clustannot_table_module$csv



fillCol(flex = c(1.5,0.15,1),
        
        fillCol(flex = c(NA,1),
                clustannot_plots_module$button,
                plotOutput("clustannot_plots", height = "100%", width = "100%")),
        br(),
        fillCol(flex = c(NA,0.01, 1),
                clustannot_table_module$button,
                br(),
                dataTableOutput("clustannot_table", height = "100%", width = "100%"))
        )

```

> A functional annotation plot for the clusters of the heatmap. Users can check more detailed annotation scores in the table below.


### Bi-plot {data-height=800}

```{r warnings=FALSE, message=FALSE, fig.width=6, fig.height=6}
clustering_biplotFUNC <- reactive({
...
clustering_biplot_text="The <strong>bi-plot</strong> tab shows a bi-plot for the first two principal PCA components, highlighting the main principal directions and its features."
 
clustering_biplot_module <- plotModule(
    id="clustering_biplot", func=clustering_biplotFUNC,
    plotlib = "base", info.text = clustering_biplot_text,
    pdf.width=10, pdf.height=10, res=75, title="PCA bi-plot"
)

output$clustering_biplot     <- clustering_biplot_module$render
output$clustering_biplot_pdf <- clustering_biplot_module$pdf

fillCol(flex = c(NA,1),
        clustering_biplot_module$button,
        plotOutput("clustering_biplot", height = "100%", width = "100%")
)

```

> A bi-plot for the first two principal PCA components to highlight the main principal directions and its features.
...
clustering_feature_scoreFUNC <- reactive({
    ngs <- inputData()
    if(is.null(ngs)) return(NULL)
    features=X=NULL
    if(input$hm_level=="geneset") {
...
clustering_feature_score_module <- plotModule(
    id="clustering_feature_score", func=clustering_feature_scoreFUNC,
    plotlib = "base", info.text = "A feature score table",
    pdf.width=6, pdf.height=10, res=75, title="Feature-set score"
)

output$clustering_feature_score     <- clustering_feature_score_module$render
output$clustering_feature_score_pdf <- clustering_feature_score_module$pdf

fillCol(flex = c(NA,1),
        clustering_feature_score_module$button,
        plotOutput("clustering_feature_score", height = "100%", width = "100%")
)
