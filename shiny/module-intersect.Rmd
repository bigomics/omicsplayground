...
```{r}
htmlOutput("curdata7", class="current-data")
...
<br><br>For the selected contrasts, the platform provides volcano plots and pairwise correlation plots between the profiles in the <strong>Pairs</strong> panel. Simultaneously, a Venn diagram with the number of intersecting genes between the profiles is plotted in <strong>Venn diagram</strong> panel. Details of intersecting genes is also reported in an interactive table. A more detailed scatter plot of two profiles is possible under the <strong>Two-pairs</strong> panel. Users can check the contrast heatmap of pairwise correlations under the <strong>Correlation</strong> panel. Alternatively, the <strong>Connectivity Map (CMap)</strong> shows the similarity of the contrasts profiles as a t-SNE plot.
...
observeEvent(input$cmp_level, {
    ...
    cat("<module-intersect:observeEvent2> input.cmp_level=",input$cmp_level,"\n")

    if(input$cmp_level == "geneset") fam = names(COLLECTIONS)
    if(input$cmp_level == "gene") fam = sort(c("<custom>",fam))
...
cmpGetFoldChangeMatrix <- reactive({
    ...
    if(input$cmp_level=="geneset") {
        ...
        cat("<module-intersect:cmpGetFoldChangeMatrix> head.sel.probes=",head(sel.probes),"\n")
...
cmp_scatterPlotMatrix.PLOT <- reactive({
    ...
    dbg("cmp_scatterPlotMatrix:: dim(fc0) =",dim(fc0),"\n")
    print("hello!")
    
    ##fc0 <- fc0[order(-rowSums(fc0**2)),]
    fc0 <- fc0[order(-apply(abs(fc0),1,max)),]
    fc0 <- fc0[order(-rowMeans(abs(fc0**2))),]
    dbg("cmp_scatterPlotMatrix:: head(rownames((fc0)) =",head(rownames((fc0))),"\n")
    dbg("cmp_scatterPlotMatrix:: head(rownames((fc1)) =",head(rownames((fc1))),"\n")
    ...
    dbg("cmp_scatterPlotMatrix:: dim(df) =",dim(df),"\n")
    ...
    dbg("cmp_scatterPlotMatrix:: sum(is.sel) =",sum(is.sel),"\n")
    dbg("cmp_scatterPlotMatrix:: head(label.text) =",head(label.text),"\n")
    ...
    dbg("cmp_scatterPlotMatrix:: length(sel1) =",length(sel1),"\n")
    dbg("cmp_scatterPlotMatrix:: head(sel1) = ",head(sel1),"\n")
    ...
    if(ncol(df)>=3) {
        dbg("cmp_scatterPlotMatrix:: plotting SPLOM (k=",ncol(df),")\n")
    
        dimensions = lapply(colnames(df), function(a) list(label=a, values = df[,a]))

        ## compute correlations and annotation positions
        rho = cor(df)
        ...
        yann = 1.03*(as.vector(rep(seq(1,0.02,-1/n),n)) - 0.15*1/n)
        ...
        dbg("cmp_scatterPlotMatrix:: plotting PAIRS (k=2)\n")
...
cmp_scatterPlotMatrix.opts = tagList(
    checkboxInput("cmp_splom_highlight","Highlight genes",TRUE),
    selectInput("cmp_splom_ntop","Number of top genes",c(100,500,1000,2500,999999),selected=1000)
)
...
##    download.fmt = c("pdf","png","html"),  ## scatterGL does not work
...
cmp_ctheatmap.PLOTLY <- reactive({
    ...
    plt <- heatmaply(
        R, margins = c(250, 250, NA, 0),
        ## k_col = 5, k_row = 5,
        cellnote = R, cellnote_size = 11,
...
cmp_connectivitymap.opts = tagList(
    selectInput('cmp_cmapsets2',"Dataset:", choices=NULL, multiple=TRUE),
...
cmp_fcbarplot.opts = tagList(
    checkboxInput('cmp_fcbarplot_abs','Absolute correlation')
)
