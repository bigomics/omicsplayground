...
<br> **Intersection analysis**. Compare experiments by intersecting their
genes. Find similar experiments that show similar signatures.

<br> The **Signature Map** maps your signature and connects them to
similar experiments.
...
```{r}
##
## External calls: selected_gsetmethods(), selected_gxmethods()
##
##
...
fillRow( flex=c(1,1),        
        ...
        selectInput("cmp_lfc","logFC threshold", choices=c(0,0.2,0.5,1,2,5), selected=0.2)
        )
...
contrastCmapGraph <- reactive({
    ...
    if(0) {
        cat("cmp_cgraph:: dim.fc0=",dim(fc0),"\n")
        fc1 <- cbind(fc0,fc0)
        fc1 <- fc1 + 1e-3*matrix(rnorm(length(fc1)),nrow(fc1),ncol(fc1))    
        pos <- Rtsne( t(scale(fc1)), perplexity=3, check_duplicates=FALSE)$Y
        ##pos <- svd(fc0)$v[,1:2]
        pos <- pos[1:ncol(fc0),]
    }
    g$layout <- layout_with_fr(g, weights=abs(E(g)$weight) )
    return(g)
})
