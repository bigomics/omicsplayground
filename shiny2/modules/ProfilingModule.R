...
<br> **Single-Cell Profiling**. Visualize the distribution of (inferred)
...
conditionalPanel(
    ...
    tagList(
        tipify(selectInput("pr_samplefilter","Filter samples:", choices=NULL, multiple=TRUE),
               "Filter relevant samples (cells).", placement="top", options = list(container = "body")),
        tipify(radioButtons('pr_clustmethod',NULL,c("tsne","pca"), inline=TRUE, selected="tsne"),
                "Specify a layout for the figures: t-SNE or PCA-based layout.", options = list(container = "body")),
        tipify(checkboxInput("pr_group", "group", FALSE),
...
pr_icp.opts = tagList(
    ...
    tipify(selectInput("pr_view","plot type:", choices=c("distribution","dotmap","heatmap")),
           "Specify the plot type: distribution, dotmap, or heatmap.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_refset", "reference:", choices=NULL),
           "Select a reference dataset for the cell type prediction.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_dcmethod","method:", choices=NULL),
           "Choose a method for the cell type prediction.", placement="top", options = list(container = "body"))
)
...
fillCol(moduleWidget(pr_icp_module))
...
pr_markersplot.opts = tagList(
    tipify(selectInput("pr_level","Level:", choices=c("gene","geneset")),
           "Specify the level of the marker analysis: gene or gene set level.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_features","Feature set:", choices=NULL, multiple=FALSE),
           "Select a particular functional group for the analysis.", placement="top", options = list(container = "body")),
    tipify(textInput("pr_search","Filter:"),
          "Filter markers by a specific keywords.", placement="top", options = list(container = "body"))
)
...
fillCol( moduleWidget(pr_markersplot_module))

```

> **T-SNE distribution of expression of marker genes.** Good biomarkers will show a distribution pattern strongly correlated with some phenotype. The top genes with the highest standard deviation are shown. The red color shading is proportional to the (absolute) expression of the gene in corresponding samples. 
...
pr_cna.opts = tagList(
    tipify(radioButtons("pr_cna_method",label="Method:", choices=c("sma40","inferCNV"),
                        inline=TRUE),
    ...
    tipify(selectInput("pr_cna_annotvar",label="Annotate with:", choices=NULL, multiple=FALSE),
    ...
    tipify(radioButtons("pr_cna_orderby","Order samples by:",c("clust","pc1"), inline=TRUE),
...
fillCol(moduleWidget(pr_cnaModule))
...
italk_LRPlot_module <- plotModule(
    "italk_LRPlot", italk_LRPlot.RENDER,
    title = "Ligand-Receptor plot", label="a",
    info.text = "The Ligand-Receptor plot visualizes the communication structure of ligand-receptor genes as
a circle plot. The width of the arrow represents the expression level/log fold change of the ligand; while the width of arrow head represents the expression level/log fold change of the receptor. Different color and the type of the arrow stands for whether the ligand and/or receptor are upregulated or downregulated. For further information, see iTALK R package (Wang et al., BioRxiv 2019).",
    options = tagList(
        tipify( selectInput("italk_LRPlot_ntop","ntop pairs",
...
italk_heatmap_module <- plotModule(
    "italk_heatmap", italk_heatmap.RENDER,
    title = "Expression heatmap", label="b",
    info.text = "The heatmap visualizes the expression level/log fold change of the ligand/receptor genes. For further information, see iTALK R package (Wang et al., BioRxiv 2019).",
    options = tagList(),
    pdf.width=6, pdf.height=8, res=80
)
...
italk_netview_module <- plotModule(
    "italk_netview", italk_netview.RENDER,
    title = "NetView", label="c",
    info.text = "The NetView plot visualizes the communication structure of ligand-receptor genes as a graph. The colors represent different types of cells as a structure and the width of edges represent the strength of the communication. Labels on the edges show exactly how many interactions exist between two types of cells. For further information, see iTALK R package (Wang et al., BioRxiv 2019).",
    options = tagList(
        tipify( selectInput("italk_netview_topgenes","top genes",
...
fillCol(
    flex = c(NA,1), 
    inputPanel(
        tipify( selectInput("italk_groups","Group by:", choices=""),
               "Select the phenotype parameter to divide samples into groups.",
               placement="right"),
        tipify( selectInput("italk_category","Gene category:",
    ...
    fillRow(
        ##flex = c(1,1,1),
        flex = c(1,1),
        moduleWidget(italk_LRPlot_module),
        moduleWidget(italk_heatmap_module)
        ##moduleWidget(italk_netview_module)
    )
)

```

> **Visualization of ligand-receptor interaction.** The iTALK R package is designed to profile and visualize the ligand-receptor mediated intercellular cross-talk signals from single-cell RNA sequencing data (scRNA-seq). iTALK uses a manually curated list of ligand-receptor gene pairs further classified into 4 categories based on the primary function of the ligand: cytokines/chemokines, immune checkpoint genes, growth factors, and others. See this [iTALK paper](https://www.biorxiv.org/content/10.1101/507871v1). **(a)** The Ligand-Receptor plot visualizes the communication structure of ligand-receptor genes as a circle plot. **(b)** The heatmap visualizes the expression level/log fold change of the ligand/receptor genes. 
...
monocle_getResults <- reactive({
    if(!require(monocle3)) install.packages("monocle3")
...
monocle_plotTopMarkers.MODULE <- plotModule(
    "monocle_plotTopMarkers", monocle_plotTopMarkers.RENDER, 
    title = "Top markers by group", label="a", 
    info.text = "The heatmap visualizes the expression of group-specific markers.",
    options = tagList(
        selectInput("monocle_groupby","Group by:",choices=NULL),
        selectInput("monocle_ntop","ntop:",choices=c(1,2,3,4,5,10,25),selected=5)
    ),
...
monocle_plotTrajectory.MODULE <- plotModule(
    "monocle_plotTrajectory", monocle_plotTrajectory.RENDER,
...
monocle_plotGene.opts = tagList(    
    selectInput("monocle_plotgene","Gene:",choices=NULL)
)
monocle_plotGene.MODULE <- plotModule(
    "monocle_plotGene", monocle_plotGene.RENDER,
...
fillRow(flex = c(1.2,1),
        fillCol(flex = c(NA,1),
                monocle_plotTopMarkers.MODULE$buttons,
                plotOutput('monocle_plotTopMarkers')
                ),
        fillCol(flex = c(NA,1,NA,1),
                monocle_plotTrajectory.MODULE$buttons,
                plotOutput('monocle_plotTrajectory'),
                monocle_plotGene.MODULE$buttons,
                plotOutput('monocle_plotGene')
                )
        )

```

> **Single-cell trajectory analysis**.  **(a)** Heatmap visualizing the expression of the group-specific top markers. **(b)** Single-cell trajectory analysis how cells choose between one of several possible end states. Reconstruction algorithms can robustly reveal branching trajectories, along with the genes that cells use to navigate these decisions. **(c)** Gene expression distribution for selected marker gene.
...
pr_xtab.opts <- tagList(
    tipify(selectInput("pr_xtabvar",label="x-axis:", choices=NULL, multiple=FALSE),
           "Choose a predefined phenotype group on the x-axis.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_xtabpheno",label="y-axis:", choices=NULL, multiple=FALSE),
           "Choose a predefined phenotype group on the y-axis.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_xtabgene",label="gene:", choices=NULL, multiple=FALSE),
           "Visualize the expression barplot of a gene by specifying the gene name.", placement="top", options = list(container = "body"))
...
> **Proportion plot.** Plot visualizing the overlap between two categorical variables (so-called cross tabulation). Although this feature is very suitable for a single-cell sequencing data, it provides useful information about the proportion of different cell types in samples obtained by the bulk sequencing method.
...
pr_cyto.opts = tagList(
    tipify(selectInput("pr_cytovar1",label="x-axis:", choices=NULL, multiple=FALSE),
           "Select your prefered gene on the x-axis.", placement="top", options = list(container = "body")),
    tipify(selectInput("pr_cytovar2",label="y-axis:", choices=NULL, multiple=FALSE),
           "Choose your prefered gene on the y-axis.", placement="top", options = list(container = "body"))
)
