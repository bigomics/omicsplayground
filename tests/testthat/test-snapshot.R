    ...
    if (!is.null(tabs)){
      lapply(tabs, function(tab){
        ...
        if(board == "connectivity") {
          ...
          if (tab == "Overview") {
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_normal"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
            App$run_js("$('#expression-genetable-datasets-datatable').find('table tr').eq(2).trigger('mousedown').trigger('mouseup'); ")
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_1gene"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
            App$run_js("$('#expression-gsettable-datasets-datatable').find('table tr').eq(2).trigger('mousedown').trigger('mouseup'); ")
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_1set"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
            App$run_js(generate_js_click_code("Foldchange (all)"))
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_foldchange"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
            App$run_js(generate_js_click_code("FDR table"))
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_fdr"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
            App$run_js(generate_js_click_code("static"))
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_static"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
          }
          if (tab == "Volcano by comparison" || tab == "Volcano by method") {
            App$wait_for_idle(duration = 3000, timeout = 100000)
            App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab, "_dynamic"), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
        ...
        App$expect_screenshot(name = paste0(pgx_file, "_", board, "_", tab), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
        }, error = function(e) {
      ...
      App$expect_screenshot(name = paste0(pgx_file, "_", board), threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
    }
    }, error = function(e) {
...
test_that("upload screens render correctly + compute gets triggered", {
  ...
  App$expect_screenshot(name = "upload_counts", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$run_js("$('#upload-counts_preview-load_example').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_counts_loaded", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$run_js(generate_js_click_code("Box plots"))
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_counts_loaded_2", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  # Samples
  App$run_js("$('.wizard-btn.next').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_samples", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$run_js("$('#upload-samples_preview-load_example').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_samples_loaded", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$run_js(generate_js_click_code("Distribution"))
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_samples_loaded_2", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  # Contrasts
  App$run_js("$('.wizard-btn.next').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_contrasts", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$run_js("$('#upload-contrasts_preview-autocontrast').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_contrasts_loaded", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  # QC
  App$run_js("$('.wizard-btn.next').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-zero_as_na" = TRUE)
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_zero_as_na", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-missing_plottype" = "ratio plot")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_missing_plottype", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-missing_plottype" = "missingness per sample")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_missing_plottype_2", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-missing_plottype" = "missingness across features")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_missing_plottype_3", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-missing_plottype" = "PCA of imputed data")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_missing_plottype_4", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-norm_plottype" = "histogram")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_norm_plottype_5", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-outlier_shownames" = TRUE)
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_norm_plottype_6", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-colorby_var" = "activated")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_norm_plottype_7", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  App$set_inputs("upload-checkqc-colorby_var" = "time")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_qc_norm_plottype_8", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")

  # Compute

  App$run_js("$('.wizard-btn.next').click();")
  App$wait_for_idle(duration = 3000, timeout = 60000)
  App$expect_screenshot(name = "upload_compute", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
  App$set_inputs("upload-compute-selected_name" = "test")
  App$set_inputs("upload-compute-selected_description" = "test")
  App$expect_screenshot(name = "upload_compute_2", threshold = SNAPSHOT_THRESHOLD, selector = "viewport")
