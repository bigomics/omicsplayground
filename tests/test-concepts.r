...
load(file="../pgx/gonzalez2018-nkvaccination.pgx",verbose=1)
##load(file="../pgx/GSE10846-dlbcl-mRNA-8k.pgx",verbose=1)

G <- readRDS(file="../files/gset-sparseG-XL.rds")
dim(G)
names(ngs)
...
head(res[order(res$NES,decreasing=TRUE),1:6],20)
head(res[order(res$NES),1:6],20)
