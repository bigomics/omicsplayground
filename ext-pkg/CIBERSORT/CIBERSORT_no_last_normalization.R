...
#main function
CIBERSORT.n <- function(sig_matrix, mixture_file, perm=0, QN=TRUE){
  ...
  if(P > 0) {nulldist <- sort(doPerm.n(P, X, Y)$dist)}
  ...
  #save results
  write.table(rbind(header,output), file="CIBERSORT.n-Results.txt", sep="\t", row.names=F, col.names=F, quote=F)
