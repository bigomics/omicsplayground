  ...
  moduleServer(id, function(input, output, session) {
    ...
    genes_selected <- shiny::reactive({
      ...
      if (gene.selected && !gset.selected) {
        ...
        this.feature <- genetable$rownames_selected() # rownames(df1)[genetable_rows_selected()]
    ...
    expression_plot_barplot_server(
      ...
      pgx = pgx,
      sel = genetable$rownames_selected, # genetable_rows_selected,
    ...
    gx_related_genesets <- shiny::reactive({
      ...
      ## get table
      sel.row <- genetable$rownames_selected() # genetable_rows_selected()
      if (is.null(sel.row)) {
        return(NULL)
      }
