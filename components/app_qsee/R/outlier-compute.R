qsee_outlier_compute <- function(X, Y, progress = NULL) {
  ...
  outliers <- playbase::detectOutlierSamples(
    Xc, methods = c("z.correlation", "z.distance", "z.features", "z.isoforest"), plot = FALSE
  )
  ...
  rownames(pca) <- colnames(X); colnames(pca) <- c("PC1", "PC2")
  if (!is.null(progress)) progress$set(message = "Preparing heatmap...", value = 0.85)
  nmax <- min(400L, nrow(X)); heatX <- X[head(order(-matrixStats::rowSds(X, na.rm = TRUE)), nmax), , drop = FALSE]
