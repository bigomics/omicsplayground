...
if(0) {
  ...
  playbase::gx.heatmap(pgx$X, nmax=100, col.annot=pgx$samples)
  X <- pgx$X
  Y <- pgx$samples
...
qsee_bsee_server <- function(id, rX, rY) {
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      r_contrasts <- reactive(pgx$contrasts)
      r_X <- reactive(pgx$X)
      r_samples <- reactive(pgx$samples)
      ...
      get_results <- reactive({
        ...
        H <- playbase::expandPhenoMatrix(Y)
        R <- cor( H, res$v )
        P <- c()
        for(i in 1:ncol(Y)) {
          m <- playbase::gx.limma( t(res$v), Y[,1], fdr=1, lfc=0,
            sort.by='none')
          m
        }
