...
viz.FoldChangeHeatmap <- function(pgx, comparisons = NULL, hilight = NULL,
  ...
  p1 <- playbase::plot_ggbarplot(t(F1),
    las = 3,
...
viz.ClusterMarkers <- function(pgx, pheno1, pheno2, n = NULL, pos = "tsne2d",
  ...
  p1 <- playbase::pgx.scatterPlotXY(
    posx,
    var = ph1, label.clusters = !lg1, legend = lg1,
...
viz.PhenoMaps <- function(pgx, phenotypes = NULL, pos = NULL, cex = 1, label = FALSE,
  ...
  for (ph in phenotypes) {
    ...
    p <- playbase::pgx.scatterPlotXY(
      posx,
      var = y, title = ph, cex = cex, legend.pos = legend.pos,
      hilight2 = hilight2,
...
viz.Contrasts <- function(pgx = NULL, contrasts = NULL, ntop = 10, dir = 0, pos = NULL,
  ...
  if (length(contrasts) == 0) {
    cat(
      "ERROR:: No valid contrasts. Available contrasts: ",
...
viz.FoldChangePairs <- function(pgx, comparisons = NULL, hilight = NULL,
  ...
  for (i in 1:nrow(comparisons)) {
    ...
    p2 <- playbase::pgx.scatterPlotXY(
      pos,
      var = sig, plotlib = "ggplot", cex = 1.0 * cex,
...
viz.MitoRiboQC <- function(pgx, group, srt = 0, pos = "tsne2d",
  ...
  y <- pgx$samples[, group]
  v1 <- playbase::plot_ggviolin(y, nCounts_RNA,
    group = NULL, srt = srt,
    main = "ncounts", ylab = "counts"
  )
  v2 <- playbase::plot_ggviolin(y, nFeature_RNA,
    group = NULL, srt = srt,
    main = "nfeature", ylab = "number"
  )
  v3 <- playbase::plot_ggviolin(y, percent.mito,
    group = NULL, srt = srt,
    main = "percent.mito", ylab = "percentage"
  )
  v4 <- playbase::plot_ggviolin(y, percent.ribo,
    group = NULL, srt = srt,
...
viz.VHVLusage <- function(pgx, by.pheno = "isotype", ng = 30, nmin = 1,
  ...
  B2 <- playbase::plot_ggbarplot(t(Matrix::head(VL.avg, ng)),
    col = brewer.pal(12, "Set3"), las = 3,
    beside = FALSE, xlab = "", ylab = "cumulative log-expression",
  ...
  H1 <- grid::grid.grabExpr(
    playbase::gx.splitmap(t(T1),
      split = NULL, cexCol = 0.85, scale = "none",
      main = paste("VL usage by", by.pheno)
    )
  )
  H2 <- grid::grid.grabExpr(
    playbase::gx.splitmap(t(T2),
      split = NULL, cexCol = 0.85,
      main = paste("VH usage by", by.pheno)
    )
  )
...
viz.GeneFamilies <- function(pgx, by.pheno = NULL, gset = NULL, ntop = 20, srt = 0,
  ...
  if (nlev <= 5) {
    plt <- playbase::plot_ggbarplot(
      df,
      x = "gene family", y = "value", fill = "pheno",
      add = "mean_se",
      position = ggplot2::position_dodge(width = 0.8)
    ) +
...
viz.BatchCorrectionMatrix <- function(X0, pheno, cX, cX2 = NULL, phenotype, stat = "F",
  ...
  for (i in 1:length(xlist)) {
    f1 <- playbase::pgx.PC_correlation(
      xlist[[i]], pheno1,
      nv = npca, stat = "F", plot = TRUE,
      main = paste0("PC variance (", main[i], ")")
    )
    f1 <- f1 +
      ggplot2::theme(
        plot.margin = ggplot2::margin(4, 4, 0, 4, "mm"),
...
viz.System <- function(pgx, contrast, umap, gs.umap) {
  ...
  viz._showShiny(plots,
    params = params, shared.params = shared.params,
...
viz.showFigure <- function(fig, widths = NULL, heights = NULL, page.height = NULL,
  ...
  if (plotlib == "plotly") {
    ...
    z <- viz._showGGplot(
      fig,
      title = title, subtitle = subtitle, caption = caption, tag = FALSE
    ) ## tagging in layout
  } else {
...
autoarrange_plots <- function(plotlist, widths = NULL, heights = NULL, ncol = NULL,
  ...
  if (!is.null(widths)) {
    autoarrange_plots.SIZED(
      plotlist,
      widths = widths, heights = heights, ncol = ncol,
    ...
    autoarrange_plots.UNIFORM(
      plotlist,
      ncol = ncol, page.height = page.height, tag = tag,
      plotlib = plotlib
    )
  }
}
