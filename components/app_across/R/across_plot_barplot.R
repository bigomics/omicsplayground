...
across_plot_barplot_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    barplot.RENDER <- function() {
      ...
      ## Order samples so bars are contiguous per dataset (then per sub-group and
      ## value within each dataset). Bars are coloured by dataset, or by phenotype
      ## value when a split is active.
      make_bar <- function(gene_df, show_legend) {
        ord <- order(as.character(gene_df$dataset), as.character(gene_df$color_group), gene_df$count)
        gene_df$sample <- factor(gene_df$sample, levels = unique(as.character(gene_df$sample)[ord]))
        plotly::plot_ly(
          data = gene_df,
          x = ~sample,
          y = ~count,
