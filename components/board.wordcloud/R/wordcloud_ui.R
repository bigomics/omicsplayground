...
WordCloudUI <- function(id) {
  ns <- shiny::NS(id) ## namespace
  ## Two stacked rows -- plots on top, tables below -- each taking half the
  ## board. The outer grid is what gives those rows a definite height to
  ## divide up, so the board fits the window exactly instead of overflowing.
  tabs <- bslib::layout_columns(
    col_widths = 12,
    height = "100%",
    row_heights = c(1, 1),
