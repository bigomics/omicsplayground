...
correlation_plot_scattercorr_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    plotly_scatter <- function(n_row, n_cols, markersize = 10, axis_title_pos = c(-0.1, -0.1), margin_l = 50, margin_b = 10, interplot_margin = 0.02) {
      ...
      suppressWarnings(
      all_plt <- plotly::subplot(sub_plots, nrows = n_row, margin = interplot_margin,
              titleY = FALSE, titleX = FALSE) %>%
        # Add common axis titles
        plotly::layout(
                  annotations = list(
                  list(x = axis_title_pos[1], y = 0.5, text = glue::glue("<b> Gene {ylab} Expression </b>"),
                        font = list(size = 13),
                        textangle = 270,
                        showarrow = FALSE, xref = 'paper', yref = 'paper'),
                  list(x = 0.5, y = axis_title_pos[2], text = glue::glue("<b> Gene {xlab} expression </b>"),
                        font = list(size = 13),
                        showarrow = FALSE, xref='paper', yref='paper')
                  ),
