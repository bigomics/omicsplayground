        ...
        bslib::layout_columns(
          ...
          bslib::layout_columns(
            ...
            mgsea_plot_enrichment_ui(
              ...
              info.text = "Barplot of enrichment score of enriched gene sets for each contrast and Volcano plot of effect sizes. Certain pathways can be enriched on multiple omics layers and are therefore of interest as potentially capturing pathways altered at multi-omics level and/or associated with conditions/traits of interest.",
              info.methods = "Upon selection of a contrast from the 'Contrast' drop-down menu on the right, an ordered bar plot of cumulative absolute enrichment score (x-axis) of each gene set is shown. Please select a gene set from the 'multiGSEA scores' table to view a Volcano plot of Effect-size (log2FC) vs Significance (-log10q) for each feature mapped in the selected gene set.",
            ...
            mofa_plot_mgsea_ui(
              ...
              info.text = "Scatter plot of enrichment scores of two omics data types. Pathway/genesets that are enriched in multiple datatypes are expected to exhibit higher multi.score values.",
              info.methods = "To aid improved visualization a small random noise is added as standard normal values to the enrichment scores. Pathways enriched in both datatypesa are colored in red.",
            ...
            mofa_plot_pathwayheatmap_ui(
              ...
              info.methods = "Top 40 most variable features (by standard deviation) are selected. Features similarities are determined using pearson correlation. Samples distances are determined using Euclidean distance and a dendrogram of sample clusters is generated.",
