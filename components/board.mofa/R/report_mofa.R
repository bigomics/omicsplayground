...
#'
#' @export
mofa_report_summary_ui <- function(
  id,
  label = "",
  title = "",
  info.text = "",
...
mofa_report_bullets_ui <- function(id) {
  ns <- shiny::NS(id)
  htmlOutput(ns("report_bullets"))
}

mofa_report_infographic_ui <- function(
  id,
  label = "",
  title = "",
  info.text = "",
...
mofa_report_inputs <- function(id) {
  ns <- shiny::NS(id)
  shiny::tagList(
    shiny::textAreaInput(
      ns("ai_prompt"),
...
#' @return
#' @export
mofa_report_server <- function(id,
                               pgx,
                               mofa,
                               watermark = FALSE) {
  moduleServer(id, function(input, output, session) {
    ...
    get_report <- shiny::eventReactive({
      ...
      this_mofa <- mofa()      
      rpt <- playbase::mofa.create_report(
        this_mofa, llm_model = llm_model,
        graph = NULL, annot = pgx$genes,
        ntop=100, psig=0.05,
        do.diagram = TRUE,
        userprompt = input$ai_prompt,
        format = "markdown",
        verbose = 1,
        progress = progress
      )
    ...
    output$downloadPDF <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        rpt <- get_report()
        rpt$infographic <- NULL
        rpt$infographic <- infographic_path()        
        this_mofa <- mofa()      
        full_rpt <- playbase::mofa.compile_markdown_report(
          mofa=this_mofa, report=rpt, hlevel=2, shift=TRUE)
    ...
    infographic_task <- ExtendedTask$new(function(report, model, outfile) {
      future_promise({
        outfile <- try( playbase::ai.create_infographic(
          report = report,
          model = model,
          filename = outfile,
