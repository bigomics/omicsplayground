        ...
        bslib::layout_columns(
          ...
          bslib::layout_columns(
            ...
            mofa_plot_snf_ui(
              ...
              info.text = "SNF affinity matrices. Prior to SNF, missing values (if any) are imputed using SVD2. For each data type, standard normalization is applied to each column of the input data to have mean 0 and standard deviation 1. Pairwise squared Euclidean distance are computed between all pairs of data points. Affinity matrix is then calculated from the distance matrix, using number of neighbors K=10-30 and hyperparameter alpha=0.5. Similarity Network Fusion is then calculated. It takes multiple views of a network and fuses them together to construct an overall status matrix. The learned status matrix can then be used for multiple analyses, including clustering, and classification. The heatmaps display sample correlation of pairwise Euclidea distances.",
