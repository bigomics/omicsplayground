...
#  fullH = "100%"
  ...
  settings_panel <- shiny::tabPanel(
    ...
    bslib::layout_columns(
      ...
      col_widths = 12,
      row_heights = list("auto", 1),
      bs_alert("Compute settings."),
      bslib::layout_columns(
        col_widths = c(6,6),
        dataview_html_settings_ui(
          ns("settings"),
          title = "Compute settings",
          caption = "Table summarizing the compute settings.",
          height = c("100%", TABLE_HEIGHT_MODAL),
          width = c("auto", "100%")
        ),
        dataview_html_settings_ui(
