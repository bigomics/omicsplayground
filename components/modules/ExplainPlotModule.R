...
ExplainPlotUI <- function(id) {
  ...
  shiny::actionLink(
    ns("button"),
    label = NULL,
    icon = shiny::icon("bolt"),  ## robot, star, bolt
    #icon = bsicons::bs_icon("stars", size = "1rem"),
...
ExplainPlotModule <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    dbg("[ExplainPlotModule]  id=",id)
    dbg("[ExplainPlotModule]  ns.id=",ns(id))
    ...
    observeEvent( input$button, {
      dbg("[ExplainPlotModule:button] button clicked for id=",id)
      ...
      if(!has_warned) {
        shinyalert::shinyalert(
          title = "",
          text = "Warning. The following is AI-generated. Your plot and its data will be sent to OpenAI for processing. Do you agree?",
          size = "xs",
    ...
    explain_plot <- function(chat, p, model, ..., session = getDefaultReactiveDomain()) {
      ...
      if(is.null(chat)) {
        message("[explain_plot] OPENING CHAT SERVER")
        chat <- ellmer::chat_openai(system_prompt = system_prompt_str, model="gpt-4o-mini")
        ##chat <- ellmer::chat_ollama(model = "llama3.2:3b", system_prompt = system_prompt_str)
      }
      ...
      onFlushed(function() {
        ...
        Sys.sleep(0.1)
        shinychat::chat_append( ns(chat_id), stream)
        ##beepr::beep(sound=1) 
      })
