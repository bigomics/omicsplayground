...
GetSocialMediaSiteLinks_WithShareLinks <- function(
  ...
  urls <- c(
    ...
    "renren" = paste0(
      "http://widget.renren.com/dialog/share?resourceUrl=", url, "&srcUrl=", url, "&title=",
      text, "&description=", desc
    ),
    "skype" = paste0("https://web.skype.com/share?url=", url, "&text=", text),
    "sms" = paste0("sms:", phone_number, "?body=", text),
    "surfingbird.ru" = paste0(
      "http://surfingbird.ru/share?url=", url, "&description=", desc, "&screenshot=",
      image, "&title=", title
    ),
    ...
    "twitter" = paste0(
      "https://twitter.com/intent/tweet?url=", url, "&text=", text, "&via=",
