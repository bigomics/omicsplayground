...
ClusteringUI <- function(id) {
  ...
  heatmap_info = "The 'Heatmap' is a powerful 2-way unsupervised clustering technique that simultaneously clusters the expression matrix along rows and columns, clustering similar genes and similar samples together. The tree-like dendrogram shows the 'distance' between features and the approximate groups. The column annotations show the correlation with the phenotypes."

  pca_info = "<b>Dimensionality reduction</b> is an unsupervised clustering technique that projects the samples into a lower dimensional, here 2D, space. Samples that have similar expression profiles will cluster close together. By coloring the points by condition, we can see which phenotype best explains the clustering."
          ...
          bslib::layout_column_wrap(          
            ...
            bslib::layout_column_wrap(
              ...
              clustering_plot_parcoord_ui(
                ...
                label = "a",
                width = c("100%", "100%"),
                ##height = c("calc(50vh - 108px)", TABLE_HEIGHT_MODAL)
                height = c("100%", TABLE_HEIGHT_MODAL)              
              ),
