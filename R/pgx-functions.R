                               ...
                               kclust=1 )
{
    ...
    has.cluster <- "cluster" %in% names(ngs$samples)
    if(skipifexists && has.tsne && has.cluster) {
        return(ngs)
    }
    ...
    ##sX = sX + 2*matrix(rnorm(length(X)),nrow(X),ncol(X))
    ...
    ##sv.rank=200
    if(sv.rank > 0) {
        cat("performing tSNE on reduced PCA...\n")
        svd <- irlba(sX, nv=sv.rank)
        sv <- svd$v %*% diag(svd$d[1:ncol(svd$v)]**gamma)
        ngs$tsne2d = Rtsne( sv, dim=2, perplexity=perplexity,
                           check_duplicates=FALSE, num_threads=99)$Y
        ngs$tsne3d = Rtsne( sv, dim=3, perplexity=perplexity,
                           check_duplicates=FALSE, num_threads=99)$Y
    } else {
        cat("performing tSNE on full matrix...\n")
        ngs$tsne2d = Rtsne( t(sX), dim=2, perplexity=perplexity,
                           check_duplicates=FALSE, num_threads=99)$Y
        ngs$tsne3d = Rtsne( t(sX), dim=3, perplexity=perplexity,
                           check_duplicates=FALSE, num_threads=99)$Y
    }
    rownames(ngs$tsne2d) = colnames(sX)
    rownames(ngs$tsne3d) = colnames(sX)

    ## ------------ find t-SNE clusters
    cat("determining clusters...\n")
    require(igraph)
    pos <- ngs$tsne2d[rownames(ngs$samples),]
    ##pos <- ngs$tsne3d[rownames(ngs$samples),]  ## use 3D (better?)
    pos = scale(pos) ## scale
    dim(pos)
    dist = as.dist(dist(pos)) ## use 3D distance??
