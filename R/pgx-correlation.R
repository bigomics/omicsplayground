...
{    
    ...
    if(0) {
        qgraph(res, layout = "spring", labels=nn)
        qgraph(res, layout = "spring", labels=nn, minimum=0.1)
...
{    
    ...
    if("cor" %in% what) {
        par(mar=c(10,4,4,2))
        r1 <- M[,"cor"] ## / ncol(M)
        if(nsize>0) r1 <- head(r1[order(-abs(r1))],nsize)
    ...
    if("pcor" %in% what) {
        ...
        if(nsize>0) r2 <- head(r2[order(-rowMeans(r2**2)),],nsize)
